...
Begin VB.Form frmProductDelivery 
   Caption         =   "成品发货"
   ClientHeight    =   7815
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13785
      _DataPath       =   ""
      Bands           =   "frmProductDelivery.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmProductDelivery.frx":5C4E
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmProductDelivery"
...
'            Exit Sub
'        End If
        If IIf(IsNull(rsdetail!B_PIShu), 0, rsdetail!B_PIShu) > 0 Or IIf(IsNull(rsdetail!B_kg), 0, rsdetail!B_kg) > 0 Or IIf(IsNull(rsdetail!B_kg), 0, rsdetail!B_kg) > 0 Then
        Else
        
            MsgBox "第" & i & "行件数,包数,米数三者有一个不能为空", vbInformation, "提示"
            Exit Sub
        End If
    ...
    sql1 = sql1 & "'12B016','PRO01','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
Private Sub savedetail()
    ...
    Do While Not rsdetail.EOF
        ...
        sql2 = "exec usp_ProductDeliveryinsert '" & item & "','" & id & "','" & rsdetail!B_ItemIDB & "'"
        sql2 = sql2 & ",'" & rsdetail!B_name & "','" & rsdetail!B_SeHao & "','" & rsdetail!B_Hua & "','" & rsdetail!B_Width & "','" & rsdetail!B_weight & "','" & rsdetail!B_PIShu & "'"
        sql2 = sql2 & ",'" & rsdetail!B_kg & "','" & rsdetail!B_PactCode & "','" & rsdetail!B_Clientid & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_memo & "','" & rsdetail!B_colorid & "','" & rsdetail!B_meter & "','" & rsdetail!B_Deliveryaddress & "','" & rsdetail!B_colorname & "','" & rsdetail!B_DeliveryGoods & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_orderitemid & "','" & rsdetail!B_DepartColor & "'"
...
'        End If
        ...
        If dPS <= 0 And dgj <= 0 And dMS <= 0 Then
            MsgBox "第" & i & "件数、包数、米数不可全部为0！", vbInformation, "提示"
            Exit Sub
        End If
        If Abs(Val(rsdetail!B_Waitfreight)) = 1 Then
            If Val(rsdetail!B_Freight) <= 0 Then
...
Private Sub MoveFirst()
    ...
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B016' and  B_BillType='PRO01'"
...
Private Sub MovePrevious()
    ...
    sql = sql & " from G_BillProduct a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID"
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B016' and  B_BillType='PRO01' and B_ID<'" & id & "'"
...
Public Sub openbill()
   ...
   sql = sql & " B_sid,B_ItemID,B_ItemIDB,B_GoodsNameAlias,B_SeHao,B_Producer,B_width,B_weight,B_ps,B_KG,a.B_PactCode,a.B_Clientid,b.B_ClientName,B_Waitfreight,B_MemoDetail,B_meter,B_Deliveryaddress,B_Deliverygoods,B_freight,B_Prepaidfreight,B_departColor"
...
Private Sub saveupdate1()
    ...
    If ComboBox3.Text = "" Then
        b = ""
    End If
    sql1 = "exec usp_ProductDelivery_update  '" & id & "','" & FlatEdit3.Text & "','" & a & "','PRO',"
    sql1 = sql1 & "'12B016','PRO01','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
    sql1 = sql1 & ",'" & FlatEdit6.Text & "','" & FlatEdit4.Text & "','" & ComboBox1.Text & "','" & Originalsuppliers & "','" & fh & "','" & ComboBox2.Text & "','" & FlatEdit7.Text & "','" & b & "','" & FlatEdit8.Text & "'"
    Gm.cnnTool.cnn.Execute sql1
    savedetailupdate
...
Private Sub savedetailupdate()
    ...
    Do While Not rsdetail.EOF
        ...
        If rs3.RecordCount > 0 Then
            sql2 = "exec usp_ProductDeliverydetailupdate '" & rsdetail!B_itemid & "','" & id & "','" & rsdetail!B_ItemIDB & "'"
            sql2 = sql2 & ",'" & rsdetail!B_name & "','" & rsdetail!B_SeHao & "','" & rsdetail!B_Hua & "','" & rsdetail!B_Width & "','" & rsdetail!B_weight & "','" & rsdetail!B_PIShu & "'"
            sql2 = sql2 & ",'" & rsdetail!B_kg & "','" & rsdetail!B_PactCode & "','" & rsdetail!B_Clientid & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_memo & "','" & rsdetail!B_colorid & "','" & rsdetail!B_meter & "','" & rsdetail!B_Deliveryaddress & "','" & rsdetail!B_colorname & "','" & rsdetail!B_DeliveryGoods & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_DepartColor & "'"
