...
Begin VB.Form frmColorPosition 
   Caption         =   "色布转库单"
   ClientHeight    =   7935
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13996
      _DataPath       =   ""
      Bands           =   "frmColorPosition.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmColorPosition.frx":53FA
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmColorPosition"
...
Private Sub setRs()
    ...
    rsdetail.Fields.Append "B_factoryColor", adVarChar, 100
    ...
    rsdetail.Fields.Append "B_Grossweight", adVarChar, 100
...
'    TDBGrid1.Columns("B_Memo").Caption = ""
   ...
   TDBGrid1.Columns("B_Grossweight").NumberFormat = "0.0"
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S077' and B_FieldName='" & szFieldName & "'"
...
Private Sub TDBGridAddOneRow(ByVal vItemID As Long)
    ...
    rsdetail!B_ItemIDB = IIf(IsNull(rs!B_ItemIDB), "", rs!B_ItemIDB)
    rsdetail!B_depart = IIf(IsNull(rs!B_ClientName), "", rs!B_ClientName)
    rsdetail!B_colorname = rs!B_GoodsNameAlias
    rsdetail!B_specification = IIf(IsNull(rs!gg), "", rs!gg)
...
Private Sub save()
    ...
    sql1 = "exec usp_ColorOrderInsert  '" & id & "','" & FlatEdit3.Text & "','" & a & "','CLC',"
    sql1 = sql1 & "'12B008','COL24 ','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
'        End If
        sql2 = "exec usp_InsertColorPosition '" & item & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_departid & "'"
        ...
        sql2 = sql2 & ",'" & rsdetail!B_Clientid & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_type & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_processid & "','" & rsdetail!B_memo & "','" & "" & "','" & rsdetail!B_repaircost & "','" & rsdetail!B_Grossweight & "','" & rsdetail!B_factoryColor & "','','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_color & "','" & rsdetail!B_orderitemid & "','" & rsdetail!B_DepartColor & "','" & rsdetail!B_Cang & "','" & rsdetail!B_beforeCang & "'"
...
Private Sub saveandprint()
    ...
    Do While Not rsdetail.EOF
        ...
        If IIf(IsNull(rsdetail!B_weight), "", rsdetail!B_weight) = "" Or rsdetail!B_weight = 0 Then
            ...
            Exit Sub
        End If
        If rsdetail!B_Waitfreight = 1 Then
            If Val(rsdetail!B_Freight) <= 0 Then
                MsgBox "第" & i & "行不能运费为0", vbInformation, "提示"
                Exit Sub
            End If
...
Private Sub sumall()
    ...
    Do While Not rs.EOF
        ...
        h = h + IIf(IsNull(Val(rs!B_Grossweight)), 0, Val(rs!B_Grossweight))
        rs.movenext
    Loop
    e = Format(a, "0.0")
    f = Format(c, "0.0")
    ...
    TDBGrid1.Columns("B_Grossweight").FooterText = "" & j & ""
End Sub
'打印时保存修改
Private Sub savetoupdate1()
    ...
    If ComboBox3.Text = "" Then
        b = ""
    End If
    sql1 = "exec usp_Colororderinsert_Update  '" & id & "','" & FlatEdit3.Text & "','" & a & "','WHT',"
    sql1 = sql1 & "'12B008','COL24 ','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
'        End If
        ...
        If rs3.RecordCount > 0 Then
            sql2 = "exec usp_ColorPosition_Update '" & rsdetail!B_ItemID & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_departid & "'"
            sql2 = sql2 & ",'" & rsdetail!B_colorname & "','" & rsdetail!B_specification & "','" & rsdetail!B_colorid & "','" & rsdetail!B_SeHao & "'"
            sql2 = sql2 & ",'" & rsdetail!B_PIShu & "','" & rsdetail!B_weight & "','" & rsdetail!B_meter & "','" & rsdetail!B_Ma & "'"
            sql2 = sql2 & ",'" & rsdetail!B_Clientid & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_type & "','" & rsdetail!B_processid & "','" & rsdetail!B_memo & "','" & "" & "','" & rsdetail!B_repaircost & "','" & rsdetail!B_Grossweight & "','" & rsdetail!B_factoryColor & "','','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_color & "','" & rsdetail!B_DepartColor & "','" & rsdetail!B_Cang & "','" & rsdetail!B_beforeCang & "'"
            Gm.cnnTool.cnn.Execute sql2
        Else
            saveadddetail
        End If
...
Private Sub MoveFirst()
    ...
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B008' and  B_BillType='COL24'"
    sql = sql & "order by B_id"
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    
    If rs.RecordCount <= 0 Then
        MsgBox "无单据", vbInformation + vbOKOnly, "提示"
        Exit Sub
    End If
    id = rs!B_id
...
Private Sub MovePrevious()
    ...
    sql = sql & " from G_Billcolor a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID"
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B008' and  B_BillType='COL24' and B_ID<'" & id & "'"
...
Public Sub openbill()
   ...
   Do While Not rs.EOF
        rsdetail.AddNew
            ...
            rsdetail!B_DepartColor = IIf(IsNull(rs!B_DepartColor), "", rs!B_DepartColor)
            rsdetail!B_factoryColor = IIf(IsNull(rs!B_factoryColor), "", rs!B_factoryColor)
            ...
            rsdetail!B_Grossweight = IIf(IsNull(rs!B_Grossweight), "", rs!B_Grossweight)
            rsdetail!B_ClientName = rs!B_Client
...
Private Sub copyhang()
    ...
    rsdetail.AddNew
            ...
            rsdetail!B_Grossweight = rs!B_Grossweight
