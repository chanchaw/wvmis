...
Begin VB.Form frmOriginalPosition 
   Caption         =   "转库单"
   ClientHeight    =   7455
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13150
      _DataPath       =   ""
      Bands           =   "frmOriginalPosition.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmOriginalPosition.frx":51E4
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmOriginalPosition"
...
Private Const theObjectID As String = "12B003"  '订单单据对象编号
...
Private Sub setRs()
    ...
    rsdetail.Fields.Append "B_level", adVarChar, 100
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S073' and B_FieldName='" & szFieldName & "'"
    ...
    If frm1.bsaved = True Then
        ...
        For Each tdbgRow In frm1.TDBGrid1.SelBookmarks
            frm1.rss.Bookmark = tdbgRow
                ...
                rsdetail!B_suppliers = IIf(IsNull(frm1.rss!B_ClientName), "", frm1.rss!B_ClientName)
                rsdetail!B_suppliersid = IIf(IsNull(frm1.rss!B_Clientid), "", frm1.rss!B_Clientid)
                ...
                rsdetail!B_price = frm1.rss!B_price
...
Private Sub save()
    ...
    Do While Not rsdetail.EOF
        ...
        If rsdetail!B_qty <= 0 Or rsdetail!B_qty = "" Then
            MsgBox "第" & i & "行不能公斤为0或者为空", vbInformation, "提示"
            Exit Sub
        End If
    ...
    If id <> "" Then
        strSQL = "select * from G_Billyarn where B_ID='" & id & "'"
    ...
    sql1 = sql1 & "'12B003','YARN10','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
Private Sub savedetail()
    ...
    Do While Not rsdetail.EOF
        ...
        sql2 = "exec usp_OriginalPositionDetail '" & item & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_suppliersid & "'"
        sql2 = sql2 & ",'" & rsdetail!B_colorid & "','" & rsdetail!B_specification & "','" & rsdetail!B_qty & "','" & rsdetail!B_price & "'"
        sql2 = sql2 & ",'" & rsdetail!B_Sum & "','" & rsdetail!B_Cang & "','" & rsdetail!B_memo & "','" & rsdetail!B_processid & "','" & rsdetail!B_supplement & "','" & rsdetail!B_level & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_BoxQty & "','" & rsdetail!B_beforeCang & "'"
...
'        sql4 = "select top 1 B_id from G_billyarn order by B_id desc"
...
Private Sub savetoupdate1()
    ...
    Do While Not rsdetail.EOF
        ...
        If rs3.RecordCount > 0 Then
            sql2 = "exec usp_OriginalPositiondetail_update '" & rsdetail!B_itemid & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_suppliersid & "'"
            sql2 = sql2 & ",'" & rsdetail!B_colorid & "','" & rsdetail!B_specification & "','" & rsdetail!B_qty & "','" & rsdetail!B_price & "'"
            sql2 = sql2 & ",'" & rsdetail!B_Sum & "','" & rsdetail!B_Cang & "','" & rsdetail!B_memo & "','" & rsdetail!B_processid & "','" & rsdetail!B_supplement & "','" & rsdetail!B_level & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_BoxQty & "' ,'" & rsdetail!B_beforeCang & "'"
...
Private Sub MoveFirst()
    ...
    sql = sql & " where B_ObjectID='12B003' and  B_BillType='YARN10'"
...
Private Sub MovePrevious()
    ...
    End If
     sql = "select top 1 B_id,B_CodeID,B_Date,B_Freightnum,B_HandNum,B_CarNum,B_payment,B_Shipment,b.B_ClientName,B_HandMent,c.B_Name,B_cope,B_drivename "
    sql = sql & " from G_BillYarn a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID left outer join G_Employee c on a.B_HandMent=c.B_SID"
    sql = sql & " where B_ObjectID='12B003' and  B_BillType='YARN10' and B_ID<'" & id & "'"
...
Public Sub openbill()
   Dim sql As String
   Dim rs As New RecordSet
   sql = "select B_itemid,B_id,B_OrderCode,B_Supplier,b.B_ClientName,B_GoodsID,c.B_Name,a.B_Specification,B_Qty,B_BeforeCang,B_LotNumber,a.B_Price,B_Sum,B_MemoDetail,B_Producer,d.B_ClientName as Client,B_supplement,B_level,B_DeliveryNote,B_boxqty"
   sql = sql & "  from G_BillDetailYarn a left outer join G_ContactCompany b on a.B_Supplier=b.B_ClientID left outer join G_Yarn c on a.B_GoodsID=c.B_SID"
   sql = sql & " left outer join G_ContactCompany d on a.B_Producer=d.B_ClientID"
   ...
   Do While Not rs.EOF
        ...
        rsdetail!B_process = IIf(IsNull(rs!client), "", rs!client)
        rsdetail!B_suppliersid = rs!B_supplier
        rsdetail!B_colorid = rs!B_GoodsID
        rsdetail!B_processid = rs!B_Producer
        rsdetail!B_supplement = IIf(IsNull(rs!B_supplement), "", rs!B_supplement)
        rsdetail!B_level = IIf(IsNull(rs!B_level), "", rs!B_level)
