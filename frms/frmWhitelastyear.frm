...
Begin VB.Form frmWhitelastyear 
   Caption         =   "白坯上年度库存"
   ClientHeight    =   7665
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13520
      _DataPath       =   ""
      Bands           =   "frmWhitelastyear.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmWhitelastyear.frx":51E4
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmWhitelastyear"
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S048' and B_FieldName='" & szFieldName & "'"
...
Private Sub AddNew()
   Dim itemidb As String
    ...
    Dim frm1 As New frmWhitelastyear_Edit
...
'    rsDetail!B_supplier = B_ClientName
'    rsDetail!B_supplierid = B_Clientid
...
'            Exit Sub
'        End If
'        If rsDetail!B_weight = "" Then
'            MsgBox "第" & i & "行克重不能为空", vbInformation, "提示"
'            Exit Sub
'        End If
        If rsdetail!B_Width = "" Then
            MsgBox "第" & i & "行门幅不能为空", vbInformation, "提示"
            Exit Sub
        End If
        rsdetail.movenext
        i = i + 1
    ...
    sql1 = sql1 & "'12B006','WHT02 ','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
Private Sub savedetail()
    ...
    Do While Not rsdetail.EOF
        ...
        sql2 = "exec usp_whitelastyeardetail '" & item & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_supplierid & "'"
        sql2 = sql2 & ",'" & rsdetail!B_whiteid & "','" & rsdetail!B_Width & "','" & rsdetail!B_weight & "','" & rsdetail!B_PIShu & "'"
        sql2 = sql2 & ",'" & rsdetail!B_kg & "','" & rsdetail!B_Cang & "','" & rsdetail!B_memo & "','" & rsdetail!B_YarnSpecification & "'"
        Debug.Print sql2
...
Private Sub savetoupdate1()
    ...
    Do While Not rsdetail.EOF
        ...
        If rs3.RecordCount > 0 Then
            sql2 = "exec usp_whitelastyeardetail_update '" & rsdetail!B_ItemID & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_supplierid & "'"
            sql2 = sql2 & ",'" & rsdetail!B_whiteid & "','" & rsdetail!B_Width & "','" & rsdetail!B_weight & "','" & rsdetail!B_PIShu & "'"
            sql2 = sql2 & ",'" & rsdetail!B_kg & "','" & rsdetail!B_Cang & "','" & rsdetail!B_memo & "','" & rsdetail!B_YarnSpecification & "'"
...
'        End If
    ...
    End If
        ...
        Dim sql3 As String
        sql3 = "exec usp_whitelastyearprint '" & id & "','" & Gm.SysID.SystemUserName & "'"
        rs3.Open sql3, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
        
        Dim frm1 As New frmModBLRPreviewOri
        Set frm1.RecordSet = rs3.Clone
            
        frm1.ObjectID = "22B065"
        frm1.Show
...
Private Sub MoveFirst()
    ...
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B006' and  B_BillType='WHT02'"
...
Private Sub MovePrevious()
    ...
    sql = sql & " from G_BillWhite a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID"
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B006' and  B_BillType='WHT02' and B_ID<'" & id & "'"
...
Public Sub openbill()
   Dim sql As String
   Dim rs As New RecordSet
   sql = "select B_ItemID,B_ItemIDB,B_Producer,b.B_ClientName as B_client,B_GoodsID,c.B_Name,B_Width,B_UnitWeight,B_pishu,B_kg,B_cang,B_Supplier,d.B_ClientName,B_MemoDetail,B_supplement,a.B_Price,B_sum,B_YarnSpecification"
   sql = sql & " from G_BillDetailWhite a left outer join G_ContactCompany b on a.B_Producer=b.B_ClientID "
   sql = sql & " left outer join G_White c on a.B_GoodsID=c.B_SID left outer join G_ContactCompany d on a.B_Supplier=d.B_ClientID "
