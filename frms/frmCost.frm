...
Private Sub grid()
    ...
    Dim sql As String
    sql = "exec usp_Cost '" & Trim(FlatEdit3.Text) & "'"
...
'    TDBGrid1.Columns("B_Sum").Caption = "采购原料"
'    TDBGrid1.Columns("B_Sum2").Caption = "调拨原料"
'    TDBGrid1.Columns("B_Sum3").Caption = "采购白坯"
'    TDBGrid1.Columns("B_Sum4").Caption = "调拨白坯"
'    TDBGrid1.Columns("B_Sum5").Caption = "加工白坯的加工费"
'    TDBGrid1.Columns("B_Sum6").Caption = "染厂加工费"
'    TDBGrid1.Columns("B_Sum7").Caption = "深加工费"
'    TDBGrid1.Columns("B_Sum8").Caption = "采购色布金额"
'
'    TDBGrid1.Columns("B_orderCode").width = 1300
'    TDBGrid1.Columns("B_Sum2").width = 900
'    TDBGrid1.Columns("B_Sum").width = 900
'    TDBGrid1.Columns("B_Sum4").width = 900
'    TDBGrid1.Columns("B_Sum3").width = 900
    TDBGrid1.Columns("B_Sum").NumberFormat = "0.0"
    TDBGrid1.Columns("B_Sum2").NumberFormat = "0.0"
    TDBGrid1.Columns("B_Sum3").NumberFormat = "0.0"
    TDBGrid1.Columns("B_Sum4").NumberFormat = "0.0"
    TDBGrid1.Columns("B_Sum5").NumberFormat = "0.0"
    TDBGrid1.Columns("B_Sum6").NumberFormat = "0.0"
    TDBGrid1.Columns("B_Sum7").NumberFormat = "0.0"
    TDBGrid1.Columns("B_Sum8").NumberFormat = "0.0"
...
Private Sub Detail()
    ...
    Dim frm1 As New frmCost_EDit
        frm1.a = TDBGrid1.Columns(TDBGrid1.Col).DataField
        frm1.b = rss!B_OrderCode
        frm1.Caption = TDBGrid1.Columns(TDBGrid1.Col).Caption
...
Private Sub pri()
    ...
    Set frm1.RecordSet = rss.Clone
    frm1.obj = "11S081"
    frm1.ObjectID = "22B107"
    frm1.Show vbModal
    Unload frm1
End Sub
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S081' and B_FieldName='" & szFieldName & "'"
...
Dim rs As New RecordSet
    ...
    rs.MoveFirst
    Do While Not rs.EOF
        a = a + IIf(IsNull(rs!B_Sum), 0, rs!B_Sum)
        b = b + IIf(IsNull(rs!B_Sum2), 0, rs!B_Sum2)
        c = c + IIf(IsNull(rs!B_Sum3), 0, rs!B_Sum3)
        d = d + IIf(IsNull(rs!B_Sum4), 0, rs!B_Sum4)
        e = e + IIf(IsNull(rs!B_Sum5), 0, rs!B_Sum5)
        f = f + IIf(IsNull(rs!B_Sum6), 0, rs!B_Sum6)
        g = g + IIf(IsNull(rs!B_Sum7), 0, rs!B_Sum7)
        h = h + IIf(IsNull(rs!B_Sum8), 0, rs!B_Sum8)
        rs.movenext
    Loop

    TDBGrid1.Columns("B_orderCode").FooterText = "合计"
    TDBGrid1.Columns("B_Sum").FooterText = "" & a & ""
    TDBGrid1.Columns("B_Sum2").FooterText = "" & b & ""
    TDBGrid1.Columns("B_Sum3").FooterText = "" & c & ""
    TDBGrid1.Columns("B_Sum4").FooterText = "" & d & ""
    TDBGrid1.Columns("B_Sum5").FooterText = "" & e & ""
    TDBGrid1.Columns("B_Sum6").FooterText = "" & f & ""
    TDBGrid1.Columns("B_Sum7").FooterText = "" & g & ""
    TDBGrid1.Columns("B_Sum8").FooterText = "" & h & ""
End Sub
