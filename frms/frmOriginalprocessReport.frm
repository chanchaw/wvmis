...
Begin VB.Form frmOriginalprocessReport 
   Caption         =   "原料发货序时表"
   ClientHeight    =   7575
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13361
      _DataPath       =   ""
      Bands           =   "frmOriginalprocessReport.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmOriginalprocessReport.frx":2E30
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmOriginalprocessReport"
...
Private Sub cob1()
    ComboBox1.Clear
    ComboBox1.AddItem "原料发货"
    ComboBox1.AddItem "加工户退货"
    ComboBox1.AddItem "原料调拨"
...
Private Sub chose()
    ...
    End If
    If ComboBox1.Text = "原料发货" Then
        ch = "YARN04"
    End If
    If ComboBox1.Text = "加工户退货" Then
        ch = "YARN05"
    End If
      If ComboBox1.Text = "原料调拨" Then
        ch = "YARN09"
    End If
End Sub
...
Private Sub Grid()
    ...
    sql = "exec usp_OriginalprocessReport '" & a & "','" & b & "','" & Originalsuppliers & "','" & ch & "','" & Gm.SysID.SystemUserName & "','" & FlatEdit1.Text & "','" & m_DateFormat & "'"
    Debug.Print sql
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S037' and B_FieldName='" & szFieldName & "'"
...
Private Sub sumall1()
    ...
    For i = 1 To n
        Sum = Sum + Val(TDBGrid1.Columns("sum2").Text)
        
        TDBGrid1.movenext
    Next i
    TDBGrid1.Columns("sum2").FooterText = "" & Sum & ""
...
Private Sub TDBGrid1_DblClick()
    ...
    Dim sql As String
    Dim rs As New RecordSet
    sql = "select * from G_BillYarn where B_id='" & rss!B_id & "'"
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    '原料发货
    If rs!B_BillType = "YARN04" Then
        openbill rss!B_id
    End If
    '加工户退货
    If rs!B_BillType = "YARN05" Then
        openbill1 rss!B_id
    End If
    '调拨
    If rs!B_BillType = "YARN09" Then
        openbill2 rss!B_id
    End If
...
'        Exit Sub
'    End If
    Dim frm1 As New frmOriginalDelivery
    frm1.id = rs!B_id
...
'        Exit Sub
'    End If
    Dim frm1 As New frmOriginalprocessReturn
    frm1.id = rs!B_id
    frm1.FlatEdit3.Text = IIf(IsNull(rs!B_Codeid), "", rs!B_Codeid)
    frm1.DTPicker1.Value = rs!B_Date
    frm1.FlatEdit2.Text = IIf(IsNull(rs!B_Freightnum), "", rs!B_Freightnum)
...
'        Exit Sub
'    End If
    Dim frm1 As New frmOriginaltransfers
    frm1.id = rs!B_id
    frm1.FlatEdit3.Text = IIf(IsNull(rs!B_Codeid), "", rs!B_Codeid)
    frm1.DTPicker1.Value = rs!B_Date
    frm1.FlatEdit2.Text = IIf(IsNull(rs!B_Freightnum), "", rs!B_Freightnum)
