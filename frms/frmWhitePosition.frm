...
Begin VB.Form frmWhitePosition 
   Caption         =   "转库单"
   ClientHeight    =   7455
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13150
      _DataPath       =   ""
      Bands           =   "frmWhitePosition.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmWhitePosition.frx":5354
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmWhitePosition"
...
Private Const theObjectID As String = "12B005"  '订单单据对象编号
...
Private Sub setRs()
    ...
    rsdetail.Fields.Append "B_originalitemidb", adVarChar, 100
    ...
    rsdetail.Fields.Append "B_BeforeCang", adVarChar, 100
...
'   TDBGrid1.Columns("B_process").Locked = True
   TDBGrid1.Columns("B_whiteName").Button = True
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S075' and B_FieldName='" & szFieldName & "'"
...
'        dingdan = frm1.dingdan
        ...
        For Each tdbgRow In frm1.TDBGrid1.SelBookmarks
            frm1.rss.Bookmark = tdbgRow
                ...
                rsdetail!B_whiteid = frm1.rss!B_sid
...
'        End If
        ...
        If Abs(rsdetail!B_Waitfreight) = 1 Then
            ...
            End If
        End If
        If rsdetail!B_beforeCang = "" Then
            MsgBox "第" & i & "行原仓位不能为空", vbInformation, "提示"
            Exit Sub
        End If
        If rsdetail!B_Cang = "" Then
            MsgBox "第" & i & "行仓位不能为空", vbInformation, "提示"
            Exit Sub
        End If
    ...
    sql1 = sql1 & "'12B005','WHT12 ','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
Private Sub savedetail()
    ...
    Do While Not rsdetail.EOF
        ...
        sql2 = "exec usp_whitePosition '" & item & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_supplierid & "'"
        sql2 = sql2 & ",'" & rsdetail!B_whiteid & "','" & rsdetail!B_Width & "','" & rsdetail!B_weight & "','" & rsdetail!B_PIShu & "'"
        sql2 = sql2 & ",'" & rsdetail!B_kg & "','" & rsdetail!B_Cang & "','" & rsdetail!B_processid & "','" & rsdetail!B_memo & "','" & rsdetail!B_supplement & "','" & rsdetail!B_price & "','" & rsdetail!B_Sum & "','" & rsdetail!B_qty & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_YarnSpecification & "','" & rsdetail!B_originalitemidb & "','" & rsdetail!B_Clientid & "','" & rsdetail!B_beforeCang & "'"
        Debug.Print sql2
...
Private Sub savetoupdate1()
    ...
    Do While Not rsdetail.EOF
        ...
        If rs3.RecordCount > 0 Then
            sql2 = "exec usp_whitePosition_update '" & rsdetail!B_itemid & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_supplierid & "'"
            sql2 = sql2 & ",'" & rsdetail!B_whiteid & "','" & rsdetail!B_Width & "','" & rsdetail!B_weight & "','" & rsdetail!B_PIShu & "'"
            sql2 = sql2 & ",'" & rsdetail!B_kg & "','" & rsdetail!B_Cang & "','" & rsdetail!B_processid & "','" & rsdetail!B_memo & "','" & rsdetail!B_supplement & "','" & rsdetail!B_price & "','" & rsdetail!B_Sum & "','" & rsdetail!B_qty & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_YarnSpecification & "','" & rsdetail!B_originalitemidb & "','" & rsdetail!B_Clientid & "','" & rsdetail!B_beforeCang & "'"
...
'    End If
      If ColIndex = TDBGrid1.Columns("B_qty").ColIndex Then
        TDBGrid1.Columns("B_qty").Value = Val(TDBGrid1.Columns("B_qty").Value)
    End If
...
'    frm1.ContactType = "白坯加工商,白坯供应商,本厂"
    ...
    If TDBGrid1.Columns("B_whiteName").ColIndex = ColIndex Then
        Dim frm3 As New frmpopupWhite
        frm3.Show vbModal
        rsdetail!B_whiteid = frm3.Whiteid
        rsdetail!B_whiteName = frm3.WhiteName
        Unload frm3
    End If
...
'        End If
    ...
    End If
        ...
        Dim sql3 As String
        sql3 = "exec usp_whiteprocessprint '" & id & "','" & Gm.SysID.SystemUserName & "'"
        rs3.Open sql3, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
        
        Dim frm1 As New frmModBLRPreviewOri
        Set frm1.RecordSet = rs3.Clone
            
        frm1.ObjectID = "22B101"
        frm1.Show
...
Private Sub MoveFirst()
    ...
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B005' and  B_BillType='WHT12'"
...
Private Sub MovePrevious()
    ...
    sql = sql & " from G_BillWhite a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID"
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B005' and  B_BillType='WHT12' and B_ID<'" & id & "'"
...
Public Sub openbill()
   Dim sql As String
   Dim rs As New RecordSet
   sql = "select B_ItemID,B_ItemIDB,B_Producer,b.B_ClientName as B_client,B_GoodsID,c.B_Name,B_Width,B_UnitWeight,B_pishu,B_kg,B_BeforeCang,B_cang,B_Supplier,d.B_ClientName,B_MemoDetail,B_supplement,a.B_Price,B_sum,B_qty,B_DeliveryNote,B_Waitfreight,B_freight,B_Prepaidfreight,B_YarnSpecification,B_originalitemidb,a.B_Clientid,e.B_ClientName as B_clientkh"
   ...
   Do While Not rs.EOF
       ...
       rsdetail!B_originalitemidb = IIf(IsNull(rs!B_originalitemidb), "", rs!B_originalitemidb)
       ...
       rsdetail!B_weight = rs!B_UnitWeight
        rsdetail!B_beforeCang = IIf(IsNull(rs!B_beforeCang), "", rs!B_beforeCang)
       ...
       rsdetail!B_qty = IIf(IsNull(rs!B_qty), 0, rs!B_qty)
       rsdetail!B_DeliveryNote = IIf(IsNull(rs!B_DeliveryNote), 0, rs!B_DeliveryNote)
...
Private Sub selectidb()
    If TDBGrid1.ApproxCount <= 0 Then
        Exit Sub
    End If
    rsdetail.Update
    
    Dim frm1 As New frmWhiteProcessInset_White
    frm1.id = rsdetail!B_ItemIDB
    frm1.Show vbModal
    Unload frm1
End Sub
