...
Begin VB.Form frmOriginalLastYear 
   Caption         =   "原料上年度库存"
   ClientHeight    =   7530
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13282
      _DataPath       =   ""
      Bands           =   "frmOriginalLastYear.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmOriginalLastYear.frx":51E4
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmOriginalLastYear"
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S047' and B_FieldName='" & szFieldName & "'"
...
Private Sub addnew()
    ...
    Dim frm1 As New frmOriginalLastYear_Edit
...
'            Exit Sub
'        End If
        If rsdetail!B_suppliers = "" Then
            MsgBox "第" & i & "行供应商不能为空", vbInformation, "提示"
            Exit Sub
        End If
            If rsdetail!B_specification = "" Then
            MsgBox "第" & i & "行规格不能为空", vbInformation, "提示"
            Exit Sub
        End If
    ...
    sql1 = sql1 & "'12B004','YARN01','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
Private Sub savedetail()
    ...
    Do While Not rsdetail.EOF
        ...
        sql2 = "exec usp_OriginallastyearDetail '" & item & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_suppliersid & "'"
        sql2 = sql2 & ",'" & rsdetail!B_colorid & "','" & rsdetail!B_specification & "','" & rsdetail!B_qty & "'"
        sql2 = sql2 & ",'" & rsdetail!B_Cang & "','" & rsdetail!B_memo & "','" & rsdetail!B_BoxQty & "'"
...
'        rs4.Open sql4, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
        ...
        sql3 = "exec usp_Originallastyearprint '" & id & "','" & Gm.SysID.SystemUserName & "'"
...
Private Sub TDBGrid1_ButtonClick(ByVal ColIndex As Integer)
 If TDBGrid1.Columns("B_suppliers").ColIndex = ColIndex Then
    ...
    frm1.Show vbModal
    rsdetail!B_suppliersid = frm1.Clientid
    rsdetail!B_suppliers = frm1.ClientName
    Unload frm1
    End If
End Sub
...
Private Sub sumall()
    ...
    rs.MoveFirst
    Do While Not rs.EOF
        a = a + IIf(IsNull(Val(rs!B_qty)), 0, Val(rs!B_qty))
...
Private Sub savetoupdate1()
    ...
    Do While Not rsdetail.EOF
        ...
        If rs3.RecordCount > 0 Then
            sql2 = "exec usp_Originallastyeardetail_update '" & rsdetail!B_itemid & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_suppliersid & "'"
            sql2 = sql2 & ",'" & rsdetail!B_colorid & "','" & rsdetail!B_specification & "','" & rsdetail!B_qty & "'"
            sql2 = sql2 & ",'" & rsdetail!B_Cang & "','" & rsdetail!B_memo & "','" & rsdetail!B_BoxQty & "'"
...
Private Sub MoveFirst()
    Dim rs As New RecordSet
    Dim sql As String
    sql = "select top 1 B_id,B_CodeID,B_Date,B_Freightnum,B_HandNum,B_CarNum,B_payment,B_Shipment,b.B_ClientName,B_HandMent,c.B_Name,B_cope "
    ...
    sql = sql & " where B_ObjectID='12B004' and  B_BillType='YARN01'"
   ...
   FlatEdit3.Text = IIf(IsNull(rs!B_Codeid), "", rs!B_Codeid)
    ...
    ComboBox1.Text = IIf(IsNull(rs!B_payment), "", rs!B_payment)
    ...
    fh = IIf(IsNull(rs!B_HandMent), "", rs!B_HandMent)
...
Private Sub MovePrevious()
    ...
    End If
     sql = "select top 1 B_id,B_CodeID,B_Date,B_Freightnum,B_HandNum,B_CarNum,B_payment,B_Shipment,b.B_ClientName,B_HandMent,c.B_Name,B_cope "
    sql = sql & " from G_BillYarn a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID left outer join G_Employee c on a.B_HandMent=c.B_SID"
    sql = sql & " where B_ObjectID='12B004' and  B_BillType='YARN01' and B_ID<'" & id & "'"
...
Public Sub openbill()
   Dim sql As String
   Dim rs As New RecordSet
   sql = "select B_itemid,B_id,B_OrderCode,B_Supplier,b.B_ClientName,B_GoodsID,c.B_Name,a.B_Specification,B_Qty,a.B_Price,B_Sum,B_LotNumber,B_MemoDetail,B_Producer,d.B_ClientName as Client,B_supplement,B_boxqty"
...
'        rsDetail!B_sum = rs!B_sum
        rsdetail!B_Cang = IIf(IsNull(rs!B_LotNumber), "", rs!B_LotNumber)
