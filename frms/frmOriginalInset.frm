...
Begin VB.Form frmOriginalInset 
   Caption         =   "原料入库"
   ClientHeight    =   8835
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   15584
      _DataPath       =   ""
      Bands           =   "frmOriginalInset.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmOriginalInset.frx":1150
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmOriginalInset"
...
Private Const theObjectID As String = "12B003"
...
Private Sub grid()
    ...
    sql = "exec usp_selectOriginalInset 'YARN08','" & client & "','" & cob1 & "'"
    Debug.Print sql
...
Private Sub TDBGrid1_RowColChange(LastRow As Variant, ByVal LastCol As Integer)
    If TDBGrid1.ApproxCount > 0 Then
        If rss.RecordCount > 0 Then
            ...
            Dim sql As String
            sql = "exec usp_POrder2Input '" & rss!B_id & "'"
            Debug.Print sql
...
Private Sub setgrid2()
    ...
    TDBGrid2.Columns("B_OrderCode").Caption = "定单号"
...
'    TDBGrid2.Columns("B_Audit").ValueItems.Presentation = dbgCheckBox
    ...
    TDBGrid2.Columns("B_OrderCode").Locked = True
        ...
        Do While Not rss1.EOF
            ...
            If N <> 0 Then
                If k < m + N Then
                    ...
                    'szJ = Format(Abs((Val(m) + Val(n) - Val(k)) / k), "0.00")
                    'szJ = Abs((Val(m) + Val(n) - Val(k)) / k)
                    's = szJ - Val(rs!B_Value)
                    
                    Debug.Print Format(f, "0.000000000")
                    If f > rs!B_Value Then
                        Debug.Print rs!B_Value
...
'        Dim szCodeID As String
        ...
        sql1 = "insert into G_BillYarn (B_ID,B_CodeID,B_BillType,B_Date,B_ContactCom,B_ObjectID) values('" & id & "','" & szCodeID & "','YARN02','" & f & "','" & rss1!B_Clientid & "','12B004')"
        Debug.Print sql1
...
Private Sub savedetail()
    ...
    Dim sql3 As String
    Dim rs As New RecordSet
    sql1 = "select * from G_DraftBilldetailYarn where 1=0"
    rs.Open sql1, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    rss1.MoveFirst
    Do While Not rss1.EOF
        If rss1!B_BoxQty > 0 Then
            ...
            rs.Update
            itemid = rs!B_itemid
            sql = "exec usp_insertOriginalDingdan '" & itemid & "','" & id & "','" & rss1!B_PactCode & "','" & rss1!B_OrderCode & "',"
            sql = sql & "'" & rss1!B_sid & "','" & rss1!B_specification & "','" & rss1!B_qty & "','" & rss1!B_BoxQty & "','" & rss1!B_DeliveryTime & "','" & rss1!B_itemid & "'"
            Gm.cnnTool.cnn.Execute sql
            sql2 = "update G_BillDetailYarn set B_BoxQty='" & "" & "',B_price='" & rss1!B_price & "' where B_itemid='" & rss1!B_itemid & "'"
            Gm.cnnTool.cnn.Execute sql2
        End If
        rss1.movenext
    Loop
...
Private Sub PushButton6_Click()
    ...
    If TDBGrid2.ApproxCount > 0 Then
        rss1.MoveFirst
        Do While Not rss1.EOF
            sql = "update G_BillDetailyarn set B_BoxQty='" & rss1!B_qty & "' where B_itemid='" & rss1!B_itemid & "'"
            Gm.cnnTool.cnn.Execute sql
            rss1.movenext
        Loop
        rss1.requery
