...
Begin VB.Form frmModBLRAcce 
   ...
   Begin C1SizerLibCtl.C1Elastic C1Elastic1 
      ...
      FloodStyle      =   1
      _GridInfo       =   $"frmModBLRAcce.frx":038A
      AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmModBLRAcce"
...
Private Sub PriviewGrid()
    ...
    VSFlexGrid1.ColDataType(3) = flexDTBoolean
...
Private Sub Audit()
    ...
    lAudit = VSFlexGrid1.cell(flexcpText, VSFlexGrid1.Row, clsVsFlexGrid1.GetColIndex("B_Audit"))
    lAudit = Abs(lAudit)
    ...
    lAudit = lAudit Xor 1
    strSQL = "Update G_BillAccessory Set B_Audit='" & lAudit & "' where B_ID=" & lID
    ...
    cls1.WriteInstance lID, Action.Audit
...
Private Sub LightAudit()
    On Error Resume Next
    Dim szBillType As String
    szBillType = VSFlexGrid1.cell(flexcpText, VSFlexGrid1.Row, clsVsFlexGrid1.GetColIndex("B_BillType"))
    If Len(szBillType) <= 0 Then
        Exit Sub
    End If
    
    If szBillType = ACCPLAN Then
        '是辅料计划单才需要审核
        ActiveBar2.Bands("菜单").Tools("审核").Enabled = True
    Else
        ActiveBar2.Bands("菜单").Tools("审核").Enabled = False
    End If
