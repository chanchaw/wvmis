...
Begin VB.Form frmWhiteReport 
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmWhiteReport.frx":2E2C
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmWhiteReport"
...
Private Sub ComboBox3_Click()
    If ComboBox3.Text = "账目日期" Then
        m_DateFormat = 1
    End If
    If ComboBox3.Text = "制单日期" Then
        m_DateFormat = 0
    End If
   Grid
End Sub
Private Sub Valuation()
    ComboBox3.AddItem "账目日期"
    ComboBox3.AddItem "制单日期"
    ComboBox3.Text = "制单日期"
    m_DateFormat = 0
End Sub
...
Private Sub Grid()
    ...
    sql = "exec usp_whitereport '" & a & "','" & b & "','" & FlatEdit1.Text & "','" & Originalsuppliers & "','" & Gm.SysID.SystemUserName & "','" & FlatEdit2.Text & "','" & m_DateFormat & "'"
    Debug.Print sql
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S038' and B_FieldName='" & szFieldName & "'"
...
'    lState = IIf(IsNull(TDBGrid1.Columns("B_Billtype").CellValue(Bookmark)), "", TDBGrid1.Columns("B_Billtype").CellValue(Bookmark))
'
'
'    If lState = "原料退货" Then
...
Private Sub sumall1()
    ...
    Sum2 = 0
    For i = 1 To n
        Sum = Sum + Val(TDBGrid1.Columns("B_PIShu").Text)
        Sum2 = Sum2 + Val(TDBGrid1.Columns("B_KG").Text)
       
        TDBGrid1.movenext
    Next i
    TDBGrid1.Columns("B_PIShu").FooterText = "" & Sum & ""
...
Private Sub TDBGrid1_DblClick()
    If TDBGrid1.ApproxCount <= 0 Then
        Exit Sub
    End If
    Debug.Print rss!B_id

    Dim sql As String
    Dim rs As New RecordSet
    sql = "select * from G_Billwhite where B_id='" & rss!B_id & "'"
 
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    Debug.Print rs!B_BillType
    If Trim(rs!B_BillType) = "WHT04" Then
       
        openbill rss!B_id
    End If
    If Trim(rs!B_BillType) = "WHT03" Then
        openbill2 rss!B_id
    End If
    If Trim(rs!B_BillType) = "WHT05" Then
        openbill3 rss!B_id
    End If
...
Private Sub openbill(ByVal id As String)
     Dim rs As New RecordSet
    Dim sql As String
    sql = "select  B_id,B_CodeID,B_Date,B_Freight,B_PlateNumber,B_PNumber,B_payment,B_Shipment,b.B_ClientName,B_Hand,c.B_Name,B_cope,B_drivename"
...
'     Dim rs As New RecordSet
'    Dim sql As String
'    sql = "select  B_id,B_CodeID,B_Date,B_Freightnum,B_HandNum,B_CarNum,B_payment,B_Shipment,b.B_ClientName,B_HandMent,c.B_Name,B_cope,B_drivename "
...
''        Exit Sub
''    End If
'    Dim frm1 As New frmOriginalReturn
'    frm1.id = rs!B_id
'    frm1.FlatEdit3.Text = IIf(IsNull(rs!B_Codeid), "", rs!B_Codeid)
'    frm1.DTPicker1.Value = rs!B_Date
'    frm1.FlatEdit2.Text = IIf(IsNull(rs!B_Freightnum), "", rs!B_Freightnum)
'    frm1.FlatEdit6.Text = IIf(IsNull(rs!B_HandNum), "", rs!B_HandNum)
'    frm1.FlatEdit4.Text = IIf(IsNull(rs!B_CarNum), "", rs!B_CarNum)
'    frm1.ComboBox1.Text = rs!B_payment
'    frm1.FlatEdit1.Text = IIf(IsNull(rs!B_ClientName), "", rs!B_ClientName)
'    frm1.Originalsuppliers = rs!B_Shipment
'    frm1.FlatEdit5.Text = IIf(IsNull(rs!B_name), "", rs!B_name)
'    frm1.fh = rs!B_HandMent
'    frm1.ComboBox2.Text = IIf(IsNull(rs!B_cope), "", rs!B_cope)
''    frm1.FlatEdit8.Text = IIf(IsNull(rs!B_drivename), "", rs!B_drivename)
'    frm1.openbill
'    frm1.Show
'End Sub
'打开页面显示上面内容,,,,,白坯采购入库
Private Sub openbill2(ByVal id As String)
     Dim rs As New RecordSet
    Dim sql As String
    sql = "select  B_id,B_CodeID,B_Date,B_Freight,B_PlateNumber,B_PNumber,B_payment,B_Shipment,b.B_ClientName,B_Hand,c.B_Name,B_cope"
    sql = sql & " from G_BillWhite a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID"
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_Id='" & id & "'"
'    sql = sql & "order by B_id desc"
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
...
'        Exit Sub
'    End If
    Dim frm1 As New frmWhiteProcureInset
    frm1.id = rs!B_id
    frm1.FlatEdit3.Text = IIf(IsNull(rs!B_Codeid), "", rs!B_Codeid)
    frm1.DTPicker1.Value = rs!B_Date
    frm1.FlatEdit2.Text = IIf(IsNull(rs!B_Freight), "", rs!B_Freight)
...
End Sub
'打开页面显示上面内容,,,,,白坯采购退货
Private Sub openbill3(ByVal id As String)
       Dim rs As New RecordSet
    Dim sql As String
    sql = "select  B_id,B_CodeID,B_Date,B_Freight,B_PlateNumber,B_PNumber,B_payment,B_Shipment,b.B_ClientName,B_Hand,c.B_Name,B_cope"
    sql = sql & " from G_BillWhite a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID"
...
'        Exit Sub
'    End If
    Dim frm1 As New frmWhiteProcureReturn
    frm1.id = rs!B_id
    frm1.FlatEdit3.Text = IIf(IsNull(rs!B_Codeid), "", rs!B_Codeid)
    frm1.DTPicker1.Value = rs!B_Date
    frm1.FlatEdit2.Text = IIf(IsNull(rs!B_Freight), "", rs!B_Freight)
