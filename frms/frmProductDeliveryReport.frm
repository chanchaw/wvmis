...
Begin VB.Form frmProductDeliveryReport 
   Caption         =   "成品发货序时表"
   ClientHeight    =   7605
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13414
      _DataPath       =   ""
      Bands           =   "frmProductDeliveryReport.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmProductDeliveryReport.frx":2922
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmProductDeliveryReport"
...
'    chose
    ...
    sql = "exec usp_ProductdeliveryReport '" & a & "','" & b & "','" & Originalsuppliers & "','" & FlatEdit1.Text & "','" & FlatEdit2.Text & "','" & Gm.SysID.SystemUserName & "'"
    Debug.Print sql
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S068' and B_FieldName='" & szFieldName & "'"
...
Private Sub TDBGrid1_DblClick()
    ...
    Dim sql As String
    Dim rs As New RecordSet
    sql = "select * from G_BillProduct where B_id='" & rss!B_id & "'"
 
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    Debug.Print rs!B_BillType
    If Trim(rs!B_BillType) = "PRO01" Then '色布发客户
        openbill rss!B_id
    End If
    If Trim(rs!B_BillType) = "PRO02" Then '客户退色布
        openbill1 rss!B_id
    End If
End Sub
'打开页面显示上面内容,,,,,色布发客户
...
'        Exit Sub
'    End If
    Dim frm1 As New frmProductDelivery
...
'        Exit Sub
'    End If
    Dim frm1 As New frmProductClientReturn
