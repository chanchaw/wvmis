...
Begin VB.Form frmColorReturnRepair 
   Caption         =   "色布回修退染厂"
   ClientHeight    =   7935
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13996
      _DataPath       =   ""
      Bands           =   "frmColorReturnRepair.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmColorReturnRepair.frx":5258
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmColorReturnRepair"
...
'   TDBGrid1.Columns("B_clientname").AllowSizing = False
           TDBGrid1.Columns("B_deliverynote").Visible = False
   TDBGrid1.Columns("B_deliverynote").Locked = True
   TDBGrid1.Columns("B_deliverynote").AllowSizing = False
           TDBGrid1.Columns("B_type").Visible = False
   TDBGrid1.Columns("B_type").Locked = True
   TDBGrid1.Columns("B_type").AllowSizing = False
   ...
   TDBGrid1.Columns("B_repaircost").NumberFormat = "0.00"
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S043' and B_FieldName='" & szFieldName & "'"
...
Private Sub save()
    ...
    sql1 = sql1 & "'12B008','COL13 ','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
Private Sub savedetail()
    ...
    Do While Not rsdetail.EOF
        ...
        sql2 = "exec usp_ColorReturnRepair '" & item & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_departid & "'"
        sql2 = sql2 & ",'" & rsdetail!B_colorname & "','" & rsdetail!B_specification & "','" & rsdetail!B_colorid & "','" & rsdetail!B_SeHao & "'"
        sql2 = sql2 & ",'" & rsdetail!B_PIShu & "','" & rsdetail!B_weight & "','" & rsdetail!B_meter & "','" & rsdetail!B_price & "'"
        sql2 = sql2 & ",'" & rsdetail!B_Clientid & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_type & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_processid & "','" & rsdetail!B_memo & "','" & rsdetail!B_repair & "','" & rsdetail!B_repaircost & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_color & "','" & rsdetail!B_orderitemid & "','" & rsdetail!B_DepartColor & "'"
...
Private Sub TDBGrid1_BeforeColUpdate(ByVal ColIndex As Integer, OldValue As Variant, Cancel As Integer)
    ...
    End If
     If ColIndex = TDBGrid1.Columns("B_Weight").ColIndex Then
        ...
        TDBGrid1.Columns("B_repaircost").Value = Abs(Val(TDBGrid1.Columns("B_Weight").Value)) * Abs(Val(TDBGrid1.Columns("B_price").Value))
    End If
...
Private Sub saveandprint()
    ...
    End If
        ...
        Dim sql3 As String
        sql3 = "exec usp_ColorReturnRepairprint '" & id & "','" & Gm.SysID.SystemUserName & "'"
...
Private Sub sumall()
    ...
    Do While Not rs.EOF
        ...
        d = d + IIf(IsNull(Val(rs!B_price)), 0, Val(rs!B_price))
        j = j + IIf(IsNull(Val(rs!B_repaircost)), 0, Val(rs!B_repaircost))
    ...
    TDBGrid1.Columns("B_price").FooterText = "" & g & ""
    TDBGrid1.Columns("B_repaircost").FooterText = "" & i & ""
End Sub
'打印时保存修改
Private Sub savetoupdate1()
    ...
    Do While Not rsdetail.EOF
        ...
        If rs3.RecordCount > 0 Then
            sql2 = "exec usp_ColorReturnRepair_Update '" & rsdetail!B_itemid & "','" & id & "','" & rsdetail!B_ItemIDB & "','" & rsdetail!B_departid & "'"
            sql2 = sql2 & ",'" & rsdetail!B_colorname & "','" & rsdetail!B_specification & "','" & rsdetail!B_colorid & "','" & rsdetail!B_SeHao & "'"
            sql2 = sql2 & ",'" & rsdetail!B_PIShu & "','" & rsdetail!B_weight & "','" & rsdetail!B_meter & "','" & rsdetail!B_price & "'"
            sql2 = sql2 & ",'" & rsdetail!B_Clientid & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_type & "','" & rsdetail!B_memo & "','" & rsdetail!B_repair & "','" & rsdetail!B_repaircost & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_color & "','" & rsdetail!B_DepartColor & "'"
            Gm.cnnTool.cnn.Execute sql2
        Else
            saveadddetail
        End If
...
Private Sub MoveFirst()
    ...
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B008' and  B_BillType='COL13'"
...
Private Sub MovePrevious()
    ...
    sql = sql & " from G_Billcolor a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID"
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B008' and  B_BillType='COL13' and B_ID<'" & id & "'"
...
Public Sub openbill()
   ...
   sql = "exec usp_ColorReturnRepairbill '" & id & "'"
   rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
   ...
   Do While Not rs.EOF
        rsdetail.addnew
            ...
            End If
            rsdetail!B_ItemIDB = rs!B_ItemIDB
            If IIf(IsNull(rs!B_ClientName), "", rs!B_ClientName) <> "" Then
               rsdetail!B_depart = rs!B_ClientName
            End If
