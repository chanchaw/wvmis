...
Begin VB.Form pnFrmOrderAcce 
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"pnFrmOrderAcce.frx":0552
         AccessibleName  =   ""
         ...
         Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar22 
            ...
            _ExtentY        =   714
            _DataPath       =   ""
            Bands           =   "pnFrmOrderAcce.frx":05D3
         End
         Begin TrueOleDBGrid80.TDBGrid TDBGrid1 
            Bindings        =   "pnFrmOrderAcce.frx":1D1B
            Height          =   4800
            Left            =   30
            TabIndex        =   2
            Top             =   465
            ...
            _StyleDefs(18)  =   "SelectedStyle:id=6,.parent=1,.namedParent=36,.bgcolor=&H8000&"
            _StyleDefs(19)  =   ":id=6,.fgcolor=&H80000018&"
            ...
            _StyleDefs(32)  =   "Splits(0).SelectedStyle:id=18,.parent=6,.bgcolor=&HFF8000&"
   ...
   End
End
Attribute VB_Name = "pnFrmOrderAcce"
...
Private Const strSQLDetail As String = "dbo.usp_Rpt_GetOrders_Acce"
Private A_BillIDOrder As Long   '订单主表的字段B_ID
Private A_BillIDAcce As Long    '辅料主表的字段B_ID
Private A_Audit As Long

'初始化本单据的审核状态
Private Sub InitAudit()
    Dim rs As RecordSet
    
    A_BillIDAcce = Gm.Business.GetAccePLNID(A_BillIDOrder)
    
    strSQL = "SELECT * FROM G_BillAccessory WHERE B_ID=" & A_BillIDAcce
    ...
    If rs.RecordCount <= 0 Then
        A_Audit = 0
    Else
        A_Audit = IIf(IsNull(rs!B_Audit), 0, rs!B_Audit)
    End If
...
Private Sub setColor()
    Dim i As Long
    Dim lQty01 As Double
    Dim lQty02 As Double
    
    If TDBGrid1.ApproxCount > 0 Then
        TDBGrid1.MoveFirst
        Do While Not TDBGrid1.EOF
            lQty01 = Val(TDBGrid1.Columns("B_Qty01").Value)
            lQty02 = Val(TDBGrid1.Columns("B_Qty02").Value)
            If lQty02 < lQty01 Then
                TDBGrid1.ForeColor = vbRed
            End If
...
Private Sub Audit()
    ...
    If MsgBox(szTip, vbYesNo + vbExclamation + vbDefaultButton2, "警告") = vbNo Then
        Exit Sub
    End If
    
    lAudit = lAudit Xor 1    '异或取反
    strSQL = "Update G_BillAccessory Set B_Audit='" & lAudit & "'"
    strSQL = strSQL & " where B_ID=" & A_BillIDAcce
    Gm.cnnTool.cnn.Execute strSQL
    
    
    '向工作流实例表中写入动作
    Dim cls1 As New clsWorkFlow
    cls1.InitCls OBJECTIDACCE
    cls1.WriteInstance A_BillIDAcce, Action.Audit
    
    '刷新网格
    RefreshGrid
    SetAudtiDisplay lAudit
End Sub
...
Private Sub RefreshGrid()
    LoadObject A_BillIDOrder
End Sub

'设置审核的显示状态
Private Sub SetAudtiDisplay(ByVal vAudti As Boolean)
    If vAudti = True Then
        ActiveBar22.Bands("主菜单").Tools("审核").Caption = "取消审核"
        ActiveBar22.Bands("主菜单").Tools("审核标记").Visible = True
    Else
        ActiveBar22.Bands("主菜单").Tools("审核").Caption = "审核"
        ActiveBar22.Bands("主菜单").Tools("审核标记").Visible = False
    End If
