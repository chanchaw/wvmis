...
Begin VB.Form frmColorInsetReport 
   Caption         =   "色布收发序时表"
   ClientHeight    =   7350
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   19315
      _DataPath       =   ""
      Bands           =   "frmColorInsetReport.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmColorInsetReport.frx":336E
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmColorInsetReport"
...
Private Sub cob1()
    ComboBox1.Clear
    ComboBox1.AddItem "染厂色布入库"
    ComboBox1.AddItem "色布退染厂"
    ComboBox1.AddItem "回修退染厂"
    ComboBox1.AddItem "回修色布入库"
...
Private Sub chose()
    If ComboBox1.Text = "全部" Then
        ch = 2
    End If
    If ComboBox1.Text = "染厂色布入库" Then
        ch = "COL04"
    End If
    If ComboBox1.Text = "色布退染厂" Then
        ch = "COL06"
    End If
    If ComboBox1.Text = "回修退染厂" Then
        ch = "COL13"
    End If
    If ComboBox1.Text = "回修色布入库" Then
        ch = "COL14"
    End If
End Sub
...
Private Sub Grid()
    ...
    sql = "exec usp_colorReport '" & a & "','" & b & "','" & Originalsuppliers & "','" & ch & "','" & Gm.SysID.SystemUserName & "','" & m_DateFormat & "'"
    Debug.Print sql
...
'    TDBGrid1.MarqueeStyle = dbgHighlightRow
    ...
    TDBGrid1.Columns("B_Waitfreight").ValueItems.Presentation = dbgCheckBox
      ...
      TDBGrid1.Columns("B_Producer").Visible = False
    TDBGrid1.Columns("B_Producer").Locked = True
    TDBGrid1.Columns("B_Producer").AllowSizing = False
...
Private Sub TDBGrid1_DblClick()
    ...
    If Trim(rs!B_BillType) = "COL04" Then '染厂色布入库
        openbill rss!B_id
    End If
    If Trim(rs!B_BillType) = "COL06" Then '色布退染厂
        openbill1 rss!B_id
    End If
    If Trim(rs!B_BillType) = "COL13" Then '回修退染厂
        openbill2 rss!B_id
    End If
    If Trim(rs!B_BillType) = "COL14" Then '回修色布入库
        openbill3 rss!B_id
    End If
...
'        Exit Sub
''    End If
    Dim frm1 As New frmColorReturn
...
'        Exit Sub
'    End If
    Dim frm1 As New frmColorReturnRepair
...
'        Exit Sub
'    End If
    Dim frm1 As New frmColorRepair
