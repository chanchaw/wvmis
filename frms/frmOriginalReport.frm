...
Begin VB.Form frmOriginalReport 
   BackColor       =   &H00FFFFFF&
   Caption         =   "原料采购定单入库对比表"
   ClientHeight    =   7605
   ClientLeft      =   120
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13414
      _DataPath       =   ""
      Bands           =   "frmOriginalReport.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmOriginalReport.frx":1AAC
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmOriginalReport"
...
Private Sub cob1()
    ComboBox1.AddItem "已完成"
    ComboBox1.AddItem "未完成"
...
Private Sub choose()
    If ComboBox1.Text = "已完成" Then
        logo = 1
    End If
    If ComboBox1.Text = "未完成" Then
        logo = 0
    End If
...
Private Sub Grid()
    ...
    sql = "exec usp_Originalreport '" & FlatEdit1.Text & "','" & a & "','" & b & "','" & FlatEdit2.Text & "','" & Originalsuppliers & "','" & logo & "'"
    Debug.Print sql
...
Private Sub SetGrid()
    setGridShow
    TDBGrid1.Columns("B_qty1").NumberFormat = "0.0"
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S013' and B_FieldName='" & szFieldName & "'"
...
Private Sub setOk()
    ...
    If frm1.bsave = True Then
        a = Trim(frm1.FlatEdit1.Text)
    Else
        Exit Sub
    End If
    Unload frm1
    sql = "update G_BillYarn set B_state='0',B_instructions='" & a & "'  where B_ID='" & rss!B_id & "'"
...
Private Sub sumall()
    ...
    rs.MoveFirst
    Do While Not rs.EOF
        a = a + IIf(IsNull(rs!B_qty1), 0, rs!B_qty1)
        b = b + IIf(IsNull(rs!B_qty), 0, rs!B_qty)
    ...
    TDBGrid1.Columns("B_CodeID").FooterText = "合计"
    TDBGrid1.Columns("B_qty1").FooterText = "" & d & ""
...
Private Sub PrintYarn()
    ...
    sql = "exec usp_PrintYarnOrder '" & rss!B_id & "'"
    Debug.Print sql
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
