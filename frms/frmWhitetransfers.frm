...
Begin VB.Form frmWhitetransfers 
   Caption         =   "染厂调拨"
   ClientHeight    =   7680
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13547
      _DataPath       =   ""
      Bands           =   "frmWhitetransfers.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmWhitetransfers.frx":51E4
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmWhitetransfers"
...
Private Sub setRs()
    ...
    rsdetail.Fields.Append "B_producer", adVarChar, 100
    rsdetail.Fields.Append "B_producerid", adVarChar, 100
...
Private Sub setrsDetail()
   ...
   TDBGrid1.Columns("B_producer").Button = True
   ...
   TDBGrid1.Columns("B_itemid").Visible = False
    ...
    TDBGrid1.Columns("B_producerid").Locked = True
   TDBGrid1.Columns("B_producerid").AllowSizing = False
   TDBGrid1.Columns("B_producerid").Visible = False
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S032' and B_FieldName='" & szFieldName & "'"
...
Private Sub addnew()
    ...
    Dim frm1 As New frmWhitetransfers_Edit
...
''        khname = frm1.khname
        ...
        For Each tdbgRow In frm1.TDBGrid1.SelBookmarks
            frm1.rss.bookmark = tdbgRow
                ...
                rsdetail!B_sid = frm1.rss!B_GoodsID
...
If TDBGrid1.Columns("B_process").ColIndex = ColIndex Then
    ...
    Unload frm2
    End If
If TDBGrid1.Columns("B_producer").ColIndex = ColIndex Then
     Dim frm1 As New frmPopupDanWei
    frm1.ContactType = "染厂"
    frm1.Show vbModal
    rsdetail!B_producerid = frm1.clientid
    rsdetail!B_Producer = frm1.ClientName
    Unload frm1
    End If
...
Private Sub save()
    ...
    If TDBGrid1.ApproxCount <= 0 Then
        MsgBox "没有数据不能保存", vbInformation, "提示"
        Exit Sub
    End If
    ...
    Do While Not rsdetail.EOF
        If IIf(IsNull(rsdetail!B_process), "", rsdetail!B_process) = "" Then
            ...
            Exit Sub
        End If
         If IIf(IsNull(rsdetail!B_Producer), "", rsdetail!B_Producer) = "" Then
            MsgBox "第" & i & "行调拨染厂不能为空", vbInformation, "提示"
            Exit Sub
        End If
    ...
    sql1 = sql1 & "'12B006','WHT11 ','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
Private Sub savedetail()
    ...
    Do While Not rsdetail.EOF
        ...
        sql2 = "exec usp_whitetransfersinsert '" & item & "','" & id & "','" & rsdetail!B_ItemIDB & "'"
        sql2 = sql2 & ",'" & rsdetail!B_sid & "','" & rsdetail!B_Width & "','" & rsdetail!B_weight & "','" & rsdetail!B_PIShu & "'"
        sql2 = sql2 & ",'" & rsdetail!B_kg & "','" & rsdetail!B_processid & "','" & rsdetail!B_memo & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_producerid & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_Clientid & "'"
...
Private Sub setOk()
    ...
    Do While Not rsdetail.EOF
        If Len(rsdetail!B_LoadTime) > 0 Then
            sql = "update G_whitecurly set B_dl=1 where B_LoadTime='" & rsdetail!B_LoadTime & "'"
...
'    setOk
    ...
    End If
        ...
        Dim sql3 As String
         sql3 = "exec usp_whitetransfersprint '" & id & "','" & Gm.SysID.SystemUserName & "'"
         Debug.Print sql
...
Private Sub MoveFirst()
    ...
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B006' and  B_BillType='WHT11'"
...
Private Sub MovePrevious()
    ...
    sql = sql & " from G_BillWhite a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID"
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B006' and  B_BillType='WHT11' and B_ID<'" & id & "'"
...
Private Sub openbill()
   Dim sql As String
   Dim rs As New RecordSet
   sql = "select B_ItemID,B_ItemIDB,B_GoodsID,b.B_Name,B_Width,B_UnitWeight,B_pishu,B_kg,B_Supplier,c.B_ClientName, B_MemoDetail,B_Waitfreight,B_transfersprocess,d.B_clientname as B_client,B_freight,B_Prepaidfreight,e.B_ClientName as B_clientkh,a.B_Clientid"
   sql = sql & " from G_BillDetailWhite a left outer join G_White b on a.B_GoodsID=b.B_SID "
   sql = sql & " left outer join G_ContactCompany c  on a.B_Supplier=c.B_ClientID left outer join G_ContactCompany d  on a.B_transfersprocess=d.B_ClientID  left outer join G_ContactCompany e on a.B_Clientid=e.B_ClientID where B_ID='" & id & "'"
   ...
   Do While Not rs.EOF
       ...
       rsdetail!B_process = rs!B_ClientName
       rsdetail!B_processid = rs!B_supplier
       rsdetail!B_Freight = rs!B_Waitfreight
       rsdetail!B_Producer = IIf(IsNull(rs!B_Client), "", rs!B_Client)
       rsdetail!B_producerid = IIf(IsNull(rs!B_transfersprocess), "", rs!B_transfersprocess)
    ...
    Do While Not rsdetail.EOF
     Set rs3 = New RecordSet
        ...
        If rs3.RecordCount > 0 Then
            sql2 = "exec usp_whitetransfersupdate_Edit '" & rsdetail!B_itemid & "','" & id & "','" & rsdetail!B_ItemIDB & "'"
            sql2 = sql2 & ",'" & rsdetail!B_sid & "','" & rsdetail!B_Width & "','" & rsdetail!B_weight & "','" & rsdetail!B_PIShu & "'"
            sql2 = sql2 & ",'" & rsdetail!B_kg & "','" & rsdetail!B_processid & "','" & rsdetail!B_memo & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_producerid & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_Clientid & "'"
            Gm.cnnTool.cnn.Execute sql2
        Else
