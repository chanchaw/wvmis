...
Begin VB.Form frmWhitecontract 
   Caption         =   "白坯无合同入库"
   ClientHeight    =   7365
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   14605
      _DataPath       =   ""
      Bands           =   "frmWhitecontract.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmWhitecontract.frx":2C78
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmWhitecontract"
...
Private Sub del()
   Dim sql As String
    ...
    If TDBGrid2.Appearance > 0 Then
        If IIf(IsNull(rss!B_itemid), "", rss!B_itemid) <> "" Then
            sql1 = "select * from G_BillDetailwhite where B_itemid='" & rss!B_itemid & "'"
            rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
            If rs.RecordCount > 0 Then
                    If validation = True Then
                        sql2 = "delete from G_BillDetailwhite where B_itemid='" & rss!B_itemid & "'"
                        Gm.cnnTool.cnn.Execute sql2
                    End If
            Else
                sql = "delete from G_DraftBillDetailwhite where B_itemid='" & rss!B_itemid & "'"
...
Private Sub add()
      Dim sql1 As String
    Dim rs As New RecordSet
    sql1 = "select * from G_Billwhite where B_id='" & id & "'"
    rs.Open sql1, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    If rs.RecordCount > 0 Then
        If validation = True Then
        
            Dim frm1 As New frmWhitecontract_Edit
            frm1.id = id
            frm1.Show vbModal
            Unload frm1
...
Private Sub eunload()
    ...
    If TDBGrid2.ApproxCount > 0 Then
        sql1 = "select * from G_Billdetailwhite where B_ID='" & id & "'"
        rs.Open sql1, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
        If rs.RecordCount <= 0 Then
...
Private Sub TDBGrid2_DblClick()
    Dim frm1 As New frmWhitecontract_Edit
    If TDBGrid2.ApproxCount > 0 Then
        frm1.itemid = rss!B_itemid
        frm1.OriginalProduct = rss!B_sid
        frm1.FlatEdit1.Text = rss!B_name
        frm1.FlatEdit2.Text = rss!B_Width
        frm1.FlatEdit6.Text = rss!B_UnitWeight
        frm1.FlatEdit3.Text = rss!B_qty
        frm1.FlatEdit4.Text = rss!B_price
        frm1.FlatEdit5.Text = rss!B_sum
        frm1.DTPicker1.Value = rss!B_DeliveryTime
        frm1.FlatEdit7.Text = rss!B_MemoDetail
    End If
    frm1.id = id
    frm1.Show vbModal
    Unload frm1
...
'    sql = "select * from G_DraftBilldetailYarn where B_ID='" & id & "'"
    sql = "exec usp_selectWUWhitedetail '" & id & "'"
...
Private Sub save()
    ...
    Else
        ...
        sql = "insert into G_Billwhite (B_ID,B_CodeID,B_BillType,B_Date,B_ContactCom,B_delivery,B_Balance,B_Memo,B_UserName) values('" & id & "','" & GetCodeID & "','WHT10','" & f & "','" & Originalsuppliers & "','" & ComboBox2.Text & "','" & ComboBox3.Text & "','" & Trim(FlatEdit4.Text) & "','" & Gm.SysID.SystemUser & "')"
        Gm.cnnTool.cnn.Execute sql
        savedetail
...
Private Sub savedetail()
    Dim sql As String
     Do While Not rss.EOF
        sql = "exec usp_InsertwhiteOrder '" & rss!B_itemid & "','" & rss!B_id & "','" & rss!B_sid & "','" & rss!B_Width & "','" & rss!B_UnitWeight & "'"
        sql = sql & ",'" & rss!B_qty & "','" & rss!B_price & "','" & rss!B_sum & "','" & rss!B_DeliveryTime & "','" & rss!B_MemoDetail & "','" & rss!B_PactCode & "','" & rss!B_orderid & "'"
        Gm.cnnTool.cnn.Execute sql
        rss.movenext
    Loop
End Sub
...
Private Sub movefrist()
    ...
    Dim rs As New RecordSet
    Dim sql As String
    sql = "select top 1 * from G_BillWhite a left outer join G_BillDetailWhite b on a.B_id=b.B_ID where a.B_BillType='WHT03' and b.B_Orderid='999999'"
...
Private Sub MovePreview()
     Dim rs As New RecordSet
     Dim sql As String
     sql = "select top 1 * from G_BillWhite a left outer join G_BillDetailWhite b on a.B_id=b.B_ID where a.B_BillType='WHT03' and b.B_Orderid='999999' and a.B_ID<'" & id & "' Order by a.B_ID desc"
     rs.Open sql, Gm.cnnTool.cnn, adOpenStatic, adLockReadOnly
...
Private Sub movenext()
     Dim rs As New RecordSet
     Dim sql As String
     sql = "select top 1 * from G_BillWhite a left outer join G_BillDetailWhite b on a.B_id=b.B_ID where a.B_BillType='WHT03' and b.B_Orderid='999999' and a.B_ID>'" & id & "' Order by a.B_ID asc"
      rs.Open sql, Gm.cnnTool.cnn, adOpenStatic, adLockReadOnly
      If rs.RecordCount <= 0 Then
        movelast
...
Private Sub movelast()
    Dim rs As New RecordSet
    Dim sql As String
    sql = "select top 1 * from G_BillWhite a left outer join G_BillDetailWhite b on a.B_id=b.B_ID where a.B_BillType='WHT03' and b.B_Orderid='999999' Order by a.B_ID desc "
    rs.Open sql, Gm.cnnTool.cnn, adOpenStatic, adLockReadOnly
     If rs.RecordCount <= 0 Then
...
Private Sub Update()
    Dim sql As String
    sql = "update G_BillWhite set B_contactCom='" & Originalsuppliers & "',B_date='" & DTPicker1.Value & "'"
...
Private Sub updatedetail()
    Dim sql As String
        rss.MoveFirst
    Do While Not rss.EOF
        sql = "update G_BilldetailWhite set B_goodsid='" & rss!B_sid & "',B_width='" & rss!B_Width & "',B_UnitWeight='" & rss!B_UnitWeight & "'"
