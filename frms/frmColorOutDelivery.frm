...
Begin VB.Form frmColorOutDelivery 
   Caption         =   "外加工发货"
   ClientHeight    =   8160
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   14393
      _DataPath       =   ""
      Bands           =   "frmColorOutDelivery.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmColorOutDelivery.frx":5258
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmColorOutDelivery"
...
Private TDBDropDown1 As TrueOleDBGrid80.TDBDropDown
...
Private Sub InitFrm()
    ...
    Dim sql As String
    Dim sql1 As String
    sql = "EXEC dbo.usp_SJGPZ"
    rs.Open sql, Gm.cnnTool.cnn, adOpenStatic, adLockReadOnly

    sql1 = "SELECT B_ProcessType FROM G_ProcessType"
    rs1.Open sql1, Gm.cnnTool.cnn, adOpenStatic, adLockReadOnly
    
    bingDropDown2TDBGrid Me, TDBGrid1, "B_WhiteColor", rs
    bingDropDown2TDBGrid Me, TDBGrid1, "B_type", rs1
End Sub
'绑定下拉框2 的结算方式
Private Sub cob2()
    ComboBox2.Clear
...
'    rsDetail.Fields.Append "B_depart", adVarChar, 100
    rsdetail.Fields.Append "B_ColorName", adVarChar, 100   '色布名称
    rsdetail.Fields.Append "B_PM", adVarChar, 100   '品名，默认为色布名称
    ...
    rsdetail.Fields.Append "B_whitecolor", adVarChar, 100   '白坯色布
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S055' and B_FieldName='" & szFieldName & "'"
...
Private Sub TDBGridAddOneRow(ByVal vItemID As Long)
    Dim sql As String
    Dim rs As New RecordSet
    sql = "exec usp_ColorOrderChoose '" & vItemID & "'"
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
...
'    rsDetail!B_depart = IIf(IsNull(rs!B_ClientName), "", rs!B_ClientName)
    rsdetail!B_colorname = rs!B_GoodsNameAlias
    rsdetail!B_PM = rs!B_GoodsNameAlias
...
'    rsDetail!B_departid = rs!B_depart
    ...
    rsdetail!B_ClientName = rs!khname
    rsdetail!B_Clientid = rs!khid
    rsdetail!B_Waitfreight = 0
    rsdetail!B_whitecolor = "色布"
...
Private Sub save()
    ...
    Do While Not rsdetail.EOF
        ...
        If rsdetail!B_process = "" Then
            MsgBox "第" & i & "行深加工单位不能为空", vbInformation, "提示"
            Exit Sub
        End If
 ...
 If ComboBox3.Text = "是" Then
    ...
    sql1 = sql1 & "'12B008','COL15 ','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
'        End If
        sql2 = "exec usp_InsertColorOutDelivery '" & item & "','" & id & "','" & rsdetail!B_ItemIDB & "'"
        ...
        sql2 = sql2 & ",'" & rsdetail!B_Clientid & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_type & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_processid & "','" & rsdetail!B_memo & "','" & "" & "','" & rsdetail!B_repaircost & "','" & rsdetail!B_whitecolor & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_color & "','" & rsdetail!B_orderitemid & "','" & rsdetail!B_DepartColor & "'"
        sql2 = sql2 & ",'" & rsdetail!B_PM & "'"
...
Private Sub saveandprint()
    ...
    End If
        ...
        Dim sql3 As String
        sql3 = "exec usp_ColorOutDeliveryreport '" & id & "','" & Gm.SysID.SystemUserName & "'"
...
'        End If
        ...
        If rs3.RecordCount > 0 Then
            sql2 = "exec usp_ColorOutDelivery_Update '" & rsdetail!B_ItemID & "','" & id & "','" & rsdetail!B_ItemIDB & "'"
            sql2 = sql2 & ",'" & rsdetail!B_colorname & "','" & rsdetail!B_specification & "','" & rsdetail!B_colorid & "','" & rsdetail!B_SeHao & "'"
            sql2 = sql2 & ",'" & rsdetail!B_PIShu & "','" & rsdetail!B_weight & "','" & rsdetail!B_meter & "','" & rsdetail!B_Ma & "'"
            sql2 = sql2 & ",'" & rsdetail!B_Clientid & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_type & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_processid & "','" & rsdetail!B_memo & "','" & "" & "','" & rsdetail!B_repaircost & "','" & rsdetail!B_whitecolor & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_color & "','" & rsdetail!B_DepartColor & "'"
            Gm.cnnTool.cnn.Execute sql2
        Else
            saveadddetail
        End If
...
Private Sub MoveFirst()
    ...
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B008' and  B_BillType='COL15'"
...
Private Sub MovePrevious()
    ...
    sql = sql & " from G_Billcolor a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID"
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B008' and  B_BillType='COL15' and B_ID<'" & id & "'"
...
'            rsDetail!B_depart = IIf(IsNull(rs!B_ClientName), "", rs!B_ClientName)
            ...
            rsdetail!B_PIShu = IIf(IsNull(rs!B_ps), "", rs!B_ps)
            rsdetail!B_weight = IIf(IsNull(rs!B_kg), "", rs!B_kg)
            rsdetail!B_meter = IIf(IsNull(rs!B_meter), "", rs!B_meter)
            rsdetail!B_Ma = IIf(IsNull(rs!B_BoxQty), "", rs!B_BoxQty)
            rsdetail!B_ClientName = IIf(IsNull(rs!B_Client), "", rs!B_Client)
            rsdetail!B_DeliveryNote = IIf(IsNull(rs!B_TransfersID), "", rs!B_TransfersID)
            rsdetail!B_type = IIf(IsNull(rs!B_type), "", rs!B_type)
...
'            rsDetail!B_departid = rs!B_depart
            ...
            rsdetail!B_whitecolor = IIf(IsNull(rs!B_whitecolor), "", rs!B_whitecolor)
            rsdetail!B_PM = IIf(IsNull(rs!B_PM), "", rs!B_PM)   '手工品名
...
'使用方法：
'vFrm：要调用本方法的窗体
'vTDBGrid：需要绑定下拉框的网格控件
'vFieldName：下拉框要绑定到的网格控件的列的字段名称
'vRs：下拉框控件的数据源。Recordset
Private Sub bingDropDown2TDBGrid(ByRef vFrm As Object, ByRef vTDBGrid As TDBGrid, _
    ByVal vFieldName As String, ByRef vRs As RecordSet)
       
    Dim rs As New RecordSet
    Set rs = vRs.Clone
       
    Set TDBDropDown1 = vFrm.Controls.add("TrueOleDBGrid80.TDBDropDown", "TDBDropDownPop" & vFieldName)
       
    vTDBGrid.Columns(Trim$(vFieldName)).DropDown = TDBDropDown1
    vTDBGrid.Columns(Trim$(vFieldName)).AutoDropDown = True
    vTDBGrid.Columns(Trim$(vFieldName)).AutoCompletion = True
       
    Set TDBDropDown1.DataSource = rs
    With TDBDropDown1
        .DataField = vFieldName
        .ListField = vFieldName
        .ColumnHeaders = False
        ...
        .RowHeight = 380
    End With
    TDBDropDown1.width = vTDBGrid.Columns(Trim$(vFieldName)).width
    'TDBDropDown1.height = TDBDropDown1.height / 2 * vRs.RecordCount + 100
    TDBDropDown1.height = TDBDropDown1.height * vRs.RecordCount + 100
...
Private Sub InitDropDown(ByRef vTDBGrid As TDBGrid)
    vTDBGrid.Columns("B_CharacterUnit").ValueItems.Presentation = dbgComboBox
    ...
    vTDBGrid.Columns("B_CharacterUnit").ValueItems.add a
    vTDBGrid.Columns("B_CharacterUnit").ValueItems.add b
    vTDBGrid.Columns("B_CharacterUnit").ValueItems.MaxComboItems = 3
End Sub
