...
Begin VB.Form frmOriginalDelivery 
   Caption         =   "原料发货/领料"
   ClientHeight    =   7455
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13150
      _DataPath       =   ""
      Bands           =   "frmOriginalDelivery.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmOriginalDelivery.frx":51E4
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmOriginalDelivery"
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S029' and B_FieldName='" & szFieldName & "'"
...
'    Dim B_price As String
    ...
    Dim frm1 As New frmOriginalOrderInset_Edit
        ...
        If Len(dingdan) > 0 Then
            ...
            frm1.FlatEdit1.SetFocus
            frm1.FlatEdit1.SelStart = 0
            frm1.FlatEdit1.SelLength = Len(FlatEdit1.Text)
        End If
    frm1.Show vbModal
...
Private Sub save()
    ...
    Do While Not rsdetail.EOF
        ...
        If rsdetail!B_qty <= 0 Or rsdetail!B_qty = "" Then
            MsgBox "第" & i & "行数量不能为0或者为空", vbInformation, "提示"
            Exit Sub
        End If
    ...
    sql1 = sql1 & "'12B004','YARN04','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
Private Sub savedetail()
    ...
    Do While Not rsdetail.EOF
        ...
        sql2 = "exec usp_OriginalDelivery '" & item & "','" & id & "','" & rsdetail!B_ItemIDB & "'"
        sql2 = sql2 & ",'" & rsdetail!B_colorid & "','" & rsdetail!B_specification & "','" & rsdetail!B_qty & "'"
        sql2 = sql2 & ",'" & rsdetail!B_memo & "','" & rsdetail!B_processid & "','" & "" & "','" & rsdetail!B_BoxQty & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_suppliers & "'"
...
'        rs4.Open sql4, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
        ...
        sql3 = "exec usp_Originaldeliveryprint '" & id & "','" & Gm.SysID.SystemUserName & "'"
...
Private Sub savetoupdate1()
    ...
    Do While Not rsdetail.EOF
        ...
        If rs3.RecordCount > 0 Then
            sql2 = "exec usp_Originaldelivery_update '" & rsdetail!B_ItemID & "','" & id & "','" & rsdetail!B_ItemIDB & "'"
            sql2 = sql2 & ",'" & rsdetail!B_colorid & "','" & rsdetail!B_specification & "','" & rsdetail!B_qty & "'"
            sql2 = sql2 & ",'" & rsdetail!B_memo & "','" & rsdetail!B_processid & "','" & "" & "','" & rsdetail!B_BoxQty & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_suppliers & "'"
...
Private Sub MoveFirst()
    ...
    sql = sql & " where B_ObjectID='12B004' and  B_BillType='YARN04'"
...
Private Sub MovePrevious()
    ...
    End If
     sql = "select top 1 B_id,B_CodeID,B_Date,B_Freightnum,B_HandNum,B_CarNum,B_payment,B_Shipment,b.B_ClientName,B_HandMent,c.B_Name,B_cope,B_drivename "
    sql = sql & " from G_BillYarn a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID left outer join G_Employee c on a.B_HandMent=c.B_SID"
    sql = sql & " where B_ObjectID='12B004' and  B_BillType='YARN04' and B_ID<'" & id & "'"
...
Public Sub openbill()
   Dim sql As String
   Dim rs As New RecordSet
   sql = "select B_ItemID,B_OrderCode,B_GoodsID,b.B_Name,a.B_Specification,B_Qty,B_MemoDetail,B_Producer,c.B_ClientName,B_supplement,B_boxqty,B_DeliveryNote,a.B_suppliers"
   sql = sql & "  from G_BillDetailYarn a left outer join G_Yarn b on a.B_GoodsID=b.B_SID left outer join G_ContactCompany c"
   sql = sql & " on a.B_Producer=c.B_ClientID where B_ID='" & id & "'"
   ...
   Do While Not rs.EOF
        ...
        rsdetail!B_processid = rs!B_Producer
         rsdetail!B_suppliers = IIf(IsNull(rs!B_suppliers), "", rs!B_suppliers)
       rsdetail.Update
       rs.movenext
   Loop
   tp
