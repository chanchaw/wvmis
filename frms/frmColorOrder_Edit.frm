...
Begin VB.Form frmColorOrder_Edit 
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   12753
      _DataPath       =   ""
      Bands           =   "frmColorOrder_Edit.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmColorOrder_Edit.frx":12A2
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmColorOrder_Edit"
...
Public B_gg As String
Public B_hex As String
Public khid As String
...
Public Sub Grid()
    ...
    sql = "exec usp_ColorOrder  '" & Trim(FlatEdit1.Text) & "'"
    Debug.Print sql
...
Private Sub SetGrid()
    ...
    TDBGrid1.Columns("gg").Caption = "规格"
    TDBGrid1.Columns("B_Name").Caption = "订单颜色"
    TDBGrid1.Columns("B_SeHao").Caption = "色号"
    TDBGrid1.Columns("khname").Caption = "客户名"
     TDBGrid1.Columns("B_UserDes").Caption = "业务员"
    TDBGrid1.Columns("B_colorname").Caption = "染厂下单颜色"
    TDBGrid1.Columns("B_Hex").Caption = "颜色事例"
    TDBGrid1.Columns("B_PatternCode").Caption = "合同色号"
    
    TDBGrid1.Columns("B_ItemIDB").width = 1000
    TDBGrid1.Columns("B_ClientName").width = 1200
...
'    TDBGrid1.Columns("B_Hex").AllowSizing = False
       TDBGrid1.Columns("khid").Visible = False
    TDBGrid1.Columns("khid").Locked = True
    TDBGrid1.Columns("khid").AllowSizing = False
...
Private Sub choose()
    If TDBGrid1.ApproxCount <= 0 Then
        Exit Sub
    End If
    If IIf(IsNull(rss!B_ClientName), "", rss!B_ClientName) = "" Then
        MsgBox "染厂为空,不能选择", vbInformation, "提示"
        Exit Sub
    End If
    ...
    B_Clientid = IIf(IsNull(rss!B_ClientName), "", rss!B_ClientName)
    B_sid = rss!B_name
    B_hex = IIf(IsNull(rss!B_hex), "", rss!B_hex)
    B_gg = rss!gg
    khid = IIf(IsNull(rss!khid), "", rss!khid)
    khname = IIf(IsNull(rss!khname), "", rss!khname)
