...
Begin VB.Form frmColorSumRepoart 
   Caption         =   "色布收发存汇总"
   ClientHeight    =   7680
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13044
      _DataPath       =   ""
      Bands           =   "frmColorSumRepoart.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmColorSumRepoart.frx":19F6
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmColorSumRepoart"
...
Private Originalsuppliers As String '染厂的id
...
Private Sub Grid()
    ...
    sql = "exec usp_ColorSum '" & a & "','" & b & "','" & FlatEdit1.Text & "','" & Originalsuppliers & "','" & Gm.SysID.SystemUserName & "','" & FlatEdit3.Text & "'"
    Debug.Print sql
...
Private Sub SetGrid()
    setGridShow
'    TDBGrid1.Columns("sum1").NumberFormat = "0.00"
'    TDBGrid1.Columns("sum3").NumberFormat = "0.00"
'    TDBGrid1.Columns("sum5").NumberFormat = "0.00"
'    TDBGrid1.Columns("sum6").NumberFormat = "0.00"
 
'    TDBGrid1.Columns("rowindex").Caption = "序号"
'    TDBGrid1.Columns("B_ClientName").Caption = "染厂"
'    TDBGrid1.Columns("B_ItemIDB").Caption = "订单号"
'    TDBGrid1.Columns("sum1").Caption = "期初结余(公斤)"
'    TDBGrid1.Columns("sum2").Caption = "本期发白坯匹数"
'    TDBGrid1.Columns("sum3").Caption = "本期发白坯公斤"
'    TDBGrid1.Columns("sum4").Caption = "本期收色布匹数"
'    TDBGrid1.Columns("sum5").Caption = "本期收色布公斤"
'    TDBGrid1.Columns("sum6").Caption = "期末结余公斤"
'
'    TDBGrid1.Columns("rowindex").width = 800
'    TDBGrid1.Columns("B_ClientName").width = 1500
'    TDBGrid1.Columns("B_ItemIDB").width = 1500
'
'    TDBGrid1.Columns("sum1").width = 2000
'    TDBGrid1.Columns("sum2").width = 2000
'    TDBGrid1.Columns("sum3").width = 2000
    ...
    TDBGrid1.Columns("client").Visible = False
    TDBGrid1.Columns("client").Locked = True
    TDBGrid1.Columns("client").AllowSizing = False
...
Private Sub sumall()
    ...
    If rss.RecordCount <= 0 Then
        TDBGrid1.Columns("sum1").FooterText = "" & a & ""
        TDBGrid1.Columns("sum2").FooterText = "" & b & ""
        TDBGrid1.Columns("sum3").FooterText = "" & c & ""
        TDBGrid1.Columns("sum4").FooterText = "" & d & ""
        TDBGrid1.Columns("sum5").FooterText = "" & e & ""
        TDBGrid1.Columns("sum6").FooterText = "" & f & ""
        TDBGrid1.Columns("sum7").FooterText = "" & h & ""
        TDBGrid1.Columns("sum8").FooterText = "" & i & ""
        TDBGrid1.Columns("sum9").FooterText = "" & j & ""
        TDBGrid1.Columns("sum10").FooterText = "" & k & ""
        TDBGrid1.Columns("sum11").FooterText = "" & l & ""
        TDBGrid1.Columns("sum12").FooterText = "" & m & ""
        
    End If
    Set rs = rss.Clone
    rs.MoveFirst
    Do While Not rs.EOF
        a = a + IIf(IsNull(rs!sum1), 0, rs!sum1)
        b = b + IIf(IsNull(rs!Sum2), 0, rs!Sum2)
        c = c + IIf(IsNull(rs!Sum3), 0, rs!Sum3)
        d = d + IIf(IsNull(rs!sum4), 0, rs!sum4)
        e = e + IIf(IsNull(rs!sum5), 0, rs!sum5)
        f = f + IIf(IsNull(rs!sum6), 0, rs!sum6)
        h = h + IIf(IsNull(rs!sum7), 0, rs!sum7)
        i = i + IIf(IsNull(rs!sum8), 0, rs!sum8)
        j = j + IIf(IsNull(rs!sum9), 0, rs!sum9)
        k = k + IIf(IsNull(rs!sum10), 0, rs!sum10)
        l = l + IIf(IsNull(rs!sum11), 0, rs!sum11)
        m = m + IIf(IsNull(rs!sum12), 0, rs!sum12)
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S082' and B_FieldName='" & szFieldName & "'"
