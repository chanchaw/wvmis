...
Begin VB.Form frmModBLAccessory 
   Caption         =   "单据 - 辅料"
   ClientHeight    =   6540
   ClientLeft      =   120
   ClientTop       =   450
   ...
   EndProperty
   Icon            =   "frmModBLAccessory.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   6540
   ScaleWidth      =   10140
   ...
   Begin C1SizerLibCtl.C1Elastic C1Elastic1 
      ...
      FloodStyle      =   1
      _GridInfo       =   $"frmModBLAccessory.frx":038A
      AccessibleName  =   ""
      ...
      Begin VB.PictureBox PictureButton 
         ...
         Begin XtremeSuiteControls.PushButton UCButton1 
            ...
            ImageGap        =   11
            IconWidth       =   16
            Icon            =   "frmModBLAccessory.frx":03E2
         End
         Begin XtremeSuiteControls.PushButton UCButton1 
            Height          =   435
            Index           =   1
            ...
            UseVisualStyle  =   -1  'True
            IconWidth       =   16
            Icon            =   "frmModBLAccessory.frx":084C
         End
         Begin XtremeSuiteControls.PushButton UCButton1 
            Height          =   435
            Index           =   2
            ...
            ImageGap        =   11
            IconWidth       =   16
            Icon            =   "frmModBLAccessory.frx":0CB6
         End
      End
      ...
      Begin ActiveBar2LibraryCtl.ActiveBar2 ABMenu 
         ...
         _ExtentY        =   1323
         _DataPath       =   ""
         Bands           =   "frmModBLAccessory.frx":1120
         ...
         Begin XtremeCommandBars.ImageManager ImageManager1 
            ...
            _ExtentY        =   635
            _StockProps     =   0
            Icons           =   "frmModBLAccessory.frx":C3C8
         End
      End
      Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
         ...
         _ExtentY        =   8996
         _DataPath       =   ""
         Bands           =   "frmModBLAccessory.frx":C3E2
         ...
         Begin TrueOleDBGrid80.TDBGrid TDBGrid1 
            Bindings        =   "frmModBLAccessory.frx":C5AA
            Height          =   4515
            Left            =   660
            TabIndex        =   9
            Top             =   120
   ...
   End
End
Attribute VB_Name = "frmModBLAccessory"
...
Private Sub TDBGrid1_ButtonClick(ByVal ColIndex As Integer)
    Adodc2.RecordSet("B_OrderCode").Value = "XXXX"
End Sub
...
Private Sub LoadDropDownGrid()
    ...
    If mvarObjectID <> "12B012" Then
        Exit Sub
    End If
    
    lOrderID = IIf(IsNull(Adodc1.RecordSet!B_BelongOrderID), 0, Adodc1.RecordSet!B_BelongOrderID)
    'strSQL = "exec dbo.usp_GetOrderAccessoryPlan '" & lOrderID & "'"
    
    strSQL = "exec dbo.[usp_BLBD_GetOrderAccessoryPlan] '" & lOrderID & "'"
    Debug.Print strSQL
    Set A_rsDD = New RecordSet
    A_rsDD.Open strSQL, Gm.cnnTool.cnn, adOpenStatic, adLockReadOnly
...
Private Sub RefreshLinkedData()
    ...
    If A_rsDD.State <> adStateOpen Then
        Exit Sub
    End If
    
    If A_rsDD.RecordCount <= 0 Then
        Exit Sub
    End If
    
    A_rsDD.Filter = " B_GoodsID='" & Adodc2.RecordSet!B_GoodsID & "'"
    If A_rsDD.RecordCount <= 0 Then
        Exit Sub
    End If
    
    Adodc2.RecordSet!B_GoodsNameAlias = A_rsDD!B_GoodsNameAlias
    Adodc2.RecordSet!B_ProducerName = A_rsDD!B_ProducerName
End Sub
...
'为当前行显示打印按钮
Private Sub SetPrintButton()
'    If IsAccPlan = False Then
'        Exit Sub
'    End If
'
'    Dim dTop As Double
...
'    Dim lCount As Long
'
'    lCount = TDBGrid1.Columns("B_ProducerName").ColIndex
'
'    dTop = TDBGrid1.Top + TDBGrid1.RowTop(TDBGrid1.Row)
'    dLeft = TDBGrid1.Left + TDBGrid1.Columns(lCount).Left + TDBGrid1.Columns(lCount).Width
'
'
'    Picture2.Left = dLeft
'    Picture2.Top = dTop
...
'如果是辅料的计划单，在返回TRUE，否则返回FALSE
Private Function IsAccPlan() As Boolean
    Dim rs As New RecordSet
    strSQL = "exec dbo.usp_GetStorageBillType_Acc '" & m_ID & "'"
    ...
    If IIf(IsNull(rs!B_Mode), "", rs!B_Mode) <> "PLN" Then
        IsAccPlan = False
    Else
        IsAccPlan = True
    End If
...
'根据单据的状态显示样式
'已登帐的单据才显示“打印派工单”
Private Sub SetBillState()
    ABMenu.Bands("Band1").Tools("打印派工单").Enabled = Not clsBL1.boolIsDraft
    ABMenu.RecalcLayout
End Sub

Private Sub PrintWorkOrder()
    Dim szObjectID As String
    Dim rs As New RecordSet
    Dim szProviderID As String
    
    szProviderID = Adodc2.RecordSet!B_Producer
    
    strSQL = "exec dbo.usp_Rpt_GetAccWorkOrder '" & m_ID & "','" & szProviderID & "'"
    Debug.Print strSQL
    rs.Open strSQL, Gm.cnnTool.cnn, adOpenStatic, adLockReadOnly
    
    szObjectID = "22B018"
    
    PrintPreview szObjectID, rs
