...
Begin VB.Form frmOriginalOrderInset_Edit 
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   12012
      _DataPath       =   ""
      Bands           =   "frmOriginalOrderInset_Edit.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmOriginalOrderInset_Edit.frx":12A2
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmOriginalOrderInset_Edit"
...
Public supperliername As String
Public B_sid As String
Public B_name As String
Public B_gg As String
Public B_qty As String
Public B_price As String
Public B_memo As String
...
Private Sub grid()
    ...
    sql1 = "select * from G_Config_OneInt where B_groupname='原料_新增行数据源'"
    rs.Open sql1, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    If IIf(IsNull(rs!B_Value), 0, rs!B_Value) = 1 Then
        sql = "exec usp_OriginalInset '" & FlatEdit1.Text & "','" & FlatEdit2.Text & "'"
        rss.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
        TDBGrid1.DataSource = rss
        setgrid
    Else
        sql = "exec usp_OriginalInsetselect '" & FlatEdit1.Text & "','" & FlatEdit2.Text & "'"
        rss.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
        TDBGrid1.DataSource = rss
        setgrid
...
End Sub
Private Sub setgrid()
    TDBGrid1.Columns("B_OrderCode").Caption = "定单号"
    TDBGrid1.Columns("B_ClientName").Caption = "供应商"
    TDBGrid1.Columns("B_Name").Caption = "原料名称"
    TDBGrid1.Columns("B_Specification").Caption = "规格"
    TDBGrid1.Columns("B_Qty").Caption = "采购数量"
    TDBGrid1.Columns("sum1").Caption = "已入库数量"
    TDBGrid1.Columns("B_price").Caption = "参考单价"
    ...
    TDBGrid1.Columns("B_OrderCode").width = 1200
    TDBGrid1.Columns("B_Qty").width = 800
    TDBGrid1.Columns("sum1").width = 1200
...
Private Sub choose()
    ...
    supplierid = IIf(IsNull(rss!B_Clientid), "", rss!B_Clientid)
    supperliername = IIf(IsNull(rss!B_ClientName), "", rss!B_ClientName)
    B_sid = rss!B_sid
    B_name = rss!B_name
    B_gg = rss!B_specification
    B_qty = rss!B_qty
    B_price = rss!B_price
    B_memo = rss!B_MemoDetail
