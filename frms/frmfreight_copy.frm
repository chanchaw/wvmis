...
Begin VB.Form frmfreight_copy 
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   12039
      _DataPath       =   ""
      Bands           =   "frmfreight_copy.frx":058A
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmfreight_copy.frx":2438
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmfreight_copy"
...
Private Sub grid()
    ...
    sql = "select * from G_Freightcost where B_Freightid='" & id & "'"
...
End Sub
Private Sub setgrid()
    TDBGrid1.Columns("B_date").Caption = "日期"
    TDBGrid1.Columns("B_costpay").Caption = "付款方式"
    TDBGrid1.Columns("B_payee").Caption = "收款人"
    TDBGrid1.Columns("B_money").Caption = "金额"
    TDBGrid1.Columns("B_memo").Caption = "备注"

    TDBGrid1.Columns("B_date").width = 1800
    TDBGrid1.Columns("B_memo").width = 3000
    
    TDBGrid1.Columns("B_ID").Visible = False
   TDBGrid1.Columns("B_ID").Locked = True
   TDBGrid1.Columns("B_ID").AllowSizing = False
       TDBGrid1.Columns("B_Freightid").Visible = False
   TDBGrid1.Columns("B_Freightid").Locked = True
   TDBGrid1.Columns("B_Freightid").AllowSizing = False
...
Private Sub TDBGrid1_RowColChange(LastRow As Variant, ByVal LastCol As Integer)
        If TDBGrid1.ApproxCount <= 0 Then
            Exit Sub
        End If
        DateTimePicker1.Value = rss!B_Date
        Text1.Text = rss!B_costpay
        Text2.Text = rss!B_payee
        Text3.Text = rss!B_money
        Text4.Text = rss!B_memo
        item = rss!B_id
End Sub
'进行保存
Private Sub save()
    ...
    sql = "select * from G_Freightcost where 1=1"
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    rs.addnew
        rs!B_Date = DateTimePicker1.Value
        rs!B_costpay = Text1.Text
        rs!B_payee = Text2.Text
        rs!B_money = Val(Text3.Text)
        rs!B_memo = Text4.Text
        rs!B_Freightid = id
    rs.Update
...
Private Sub upd()
    ...
    sql = "update G_Freightcost set B_date='" & a & "',B_costpay='" & Text1.Text & "',B_payee='" & Text2.Text & "',"
    sql = sql & " B_money='" & Text3.Text & "',B_memo='" & Text4.Text & "' where B_id='" & rss!B_id & "'"
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
...
Private Sub del()
    ...
    End If
    Dim sql As String
    sql = "delete from G_Freightcost where B_id='" & rss!B_id & "'"
...
Private Sub sumall1()
    ...
    rs.MoveFirst
    Do While Not rs.EOF
        a = a + IIf(IsNull(Val(rs!B_money)), 0, Val(rs!B_money))
  
        rs.movenext
    Loop
    b = Format(a, "0.00")

    TDBGrid1.Columns("B_date").FooterText = "合计"
    TDBGrid1.Columns("B_money").FooterText = "" & b & ""
