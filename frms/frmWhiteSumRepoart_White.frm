...
Begin VB.Form frmWhiteSumRepoart_White 
   Caption         =   "白坯收发存汇总"
   ClientHeight    =   7530
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13282
      _DataPath       =   ""
      Bands           =   "frmWhiteSumRepoart_White.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmWhiteSumRepoart_White.frx":18F8
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmWhiteSumRepoart_White"
...
Private Originalsuppliers As String '白坯的id
...
Private Sub PushButton1_Click()
    ...
    frm1.Show vbModal
    FlatEdit3.Text = Trim(frm1.WhiteName)
...
Private Sub grid()
    ...
    sql = "exec usp_GoodsWhiteSum '" & a & "','" & b & "','" & Originalsuppliers & "','" & Gm.SysID.SystemUserName & "','" & FlatEdit3.Text & "'"
    Debug.Print sql
...
'    setGridShow
    ...
    TDBGrid1.Columns("sum7").NumberFormat = "0.00"
 
    TDBGrid1.Columns("rowindex").Caption = "序号"
    TDBGrid1.Columns("B_name").Caption = "白坯品名"
    TDBGrid1.Columns("sum1").Caption = "期初结余公斤"
    TDBGrid1.Columns("sum2").Caption = "期初结余匹数"
    TDBGrid1.Columns("sum3").Caption = "本期发白坯公斤"
    TDBGrid1.Columns("sum4").Caption = "本期发白坯匹数"
    TDBGrid1.Columns("sum5").Caption = "本期收色布公斤"
    TDBGrid1.Columns("sum6").Caption = "本期收色布匹数"
    TDBGrid1.Columns("sum7").Caption = "期初结余公斤"
    TDBGrid1.Columns("sum8").Caption = "期初结余匹数"
    
    TDBGrid1.Columns("rowindex").width = 800
    TDBGrid1.Columns("B_name").width = 1500
...
Private Sub sumall()
    ...
    If rss.RecordCount <= 0 Then
        ...
        TDBGrid1.Columns("sum7").FooterText = "" & k & ""
        TDBGrid1.Columns("sum8").FooterText = "" & m & ""
    End If
    Set rs = rss.Clone
    rs.MoveFirst
    Do While Not rs.EOF
        ...
        k = k + IIf(IsNull(rs!sum6), 0, rs!sum6)
        f = f + IIf(IsNull(rs!sum7), 0, rs!sum7)
        m = m + IIf(IsNull(rs!sum8), 0, rs!sum8)
        rs.movenext
    Loop
    j = Format(a, "0.0")
    h = Format(c, "0.0")
    g = Format(e, "0.0")
    i = Format(f, "0.0")
   TDBGrid1.Columns("sum1").FooterText = "" & j & ""
    TDBGrid1.Columns("sum2").FooterText = "" & b & ""
    TDBGrid1.Columns("sum3").FooterText = "" & h & ""
    TDBGrid1.Columns("sum4").FooterText = "" & d & ""
    TDBGrid1.Columns("sum5").FooterText = "" & g & ""
    TDBGrid1.Columns("sum6").FooterText = "" & k & ""
    TDBGrid1.Columns("sum7").FooterText = "" & i & ""
    TDBGrid1.Columns("sum8").FooterText = "" & m & ""
End Sub
