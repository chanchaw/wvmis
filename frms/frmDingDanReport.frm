...
Begin VB.Form frmDingDanReport 
   Caption         =   "合同审核"
   ClientHeight    =   6825
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   12039
      _DataPath       =   ""
      Bands           =   "frmDingDanReport.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmDingDanReport.frx":0F2C
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmDingDanReport"
...
Private Sub grid()
    ...
    If ComboBox1.Text = "全部" Then
        a = 2
    End If
    sql = "exec usp_DingDanReport '" & a & "','" & client & "','" & Trim(FlatEdit2.Text) & "'"
    Debug.Print sql
...
Private Sub setgrid()
    ...
    TDBGrid1.Columns("B_Date").Caption = "合同签订日"
    ...
    TDBGrid1.Columns("B_Datecreate").Caption = "合同制作时间"
    TDBGrid1.Columns("B_dateaudit").Caption = "审核时间"
...
Private Sub Aud()
    ...
    Dim sql As String
    Dim rs As New RecordSet
    sql = "update G_BillOrder set B_Audit='1' where B_id='" & rss!B_id & "'"
    Debug.Print sql
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    rss.requery
End Sub
...
Private Sub cancelAudit()
    ...
    Dim sql As String
    Dim rs As New RecordSet
    sql = "update G_BillOrder set B_Audit='0' where B_id='" & rss!B_id & "'"
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    rss.requery
End Sub
