...
Begin VB.Form frmWhiteProduction 
   Caption         =   "织造产量表"
   ClientHeight    =   7380
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13018
      _DataPath       =   ""
      Bands           =   "frmWhiteProduction.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmWhiteProduction.frx":15DC
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmWhiteProduction"
...
Private Sub Command1_Click()
    ...
    If Trim(Text2.Text) = "" Then
        MsgBox "公斤为空,不能保存", vbInformation, "提示"
        Exit Sub
    End If
    sql = "insert into G_WhiteProduction(B_id,B_kg,B_Machine) values('" & rss!B_itemid & "','" & Text2.Text & "','" & Text3.Text & "')"
...
Private Sub grid()
    ...
    sql = "exec usp_SelectWhiteProduction '" & a & "','" & b & "','" & Text1.Text & "'"
    Debug.Print sql
...
Private Sub setgrid()
    ...
    TDBGrid1.Columns("B_DateCreate").Caption = "日期"
    TDBGrid1.Columns("B_BoxQty").Caption = "计划"
    TDBGrid1.Columns("B_sum").Caption = "已完成"
    TDBGrid1.Columns("B_KG").Caption = "公斤"
    TDBGrid1.Columns("B_ItemIDB").width = 1000
    TDBGrid1.Columns("B_GoodsNameAlias").width = 2400
    TDBGrid1.Columns("B_Goodsid").width = 1400
    TDBGrid1.Columns("B_MaoHight").width = 1000
    TDBGrid1.Columns("B_Width").width = 1000
    TDBGrid1.Columns("B_UnitWeight").width = 1000
    TDBGrid1.Columns("B_DateCreate").width = 1400
    TDBGrid1.Columns("B_BoxQty").width = 1400
    TDBGrid1.Columns("B_sum").width = 1400
    TDBGrid1.Columns("B_KG").width = 1400
...
'    sumall
 TDBGrid1.Columns("B_ItemID").Visible = False
   ...
   TDBGrid1.Columns("打印时间").AllowSizing = False
      TDBGrid1.Columns("B_Item").Visible = False
   TDBGrid1.Columns("B_Item").Locked = True
   TDBGrid1.Columns("B_Item").AllowSizing = False
   
   TDBGridMergeCell TDBGrid1, "B_ItemIDB"
   TDBGridMergeCell TDBGrid1, "B_Item"
    TDBGridMergeCell TDBGrid1, "B_BoxQty"
    TDBGridMergeCell TDBGrid1, "B_sum"
   sumall
End Sub
...
Private Sub sumall()
    ...
    Do While Not rs.EOF
        ...
        b = b + IIf(IsNull(rs!B_sum), 0, rs!B_sum)
         c = c + IIf(IsNull(rs!B_kg), 0, rs!B_kg)
        rs.movenext
    Loop
'    c = Format(a, "0.0")
    d = Format(a, "0.0")
    e = Format(b, "0.0")
    f = Format(c, "0.0")
    TDBGrid1.Columns("B_BoxQty").FooterText = "" & e & ""
