...
Begin VB.Form frmWhiteDelivery 
   Caption         =   "白坯发货"
   ClientHeight    =   7455
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13150
      _DataPath       =   ""
      Bands           =   "frmWhiteDelivery.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmWhiteDelivery.frx":5BD2
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmWhiteDelivery"
...
Private Sub Form_Load()
    ...
    sql = "select * from G_Config_OneInt where B_groupName='白坯单据录入方式'"
    Debug.Print sql
...
Private Sub setRs()
    ...
    rsdetail.Fields.Append "B_ZCDate", adVarChar, 100
    ...
    rsdetail.Fields.Append "B_YarnSpecification", adVarChar, 100
...
Private Sub setrsDetail()
   ...
   TDBGrid1.Columns("B_kg").NumberFormat = "0.00"
'   TDBGrid1.Columns("B_itemidb").Locked = True
   TDBGrid1.Columns("B_name").Locked = True
'   TDBGrid1.Columns("B_width").Locked = True
'   TDBGrid1.Columns("B_weight").Locked = True
  
   TDBGrid1.Columns("B_name").Button = True
   ...
   TDBGrid1.Columns("B_ZCDate").Locked = True
   TDBGrid1.Columns("B_ZCDate").AllowSizing = False
   TDBGrid1.Columns("B_ZCDate").Visible = False
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S024' and B_FieldName='" & szFieldName & "'"
...
Private Sub AddNew()
    ...
    Dim frm1 As New frmWhiteProcessInset_Edit
...
'        dingdan = frm1.dingdan
        ...
        For Each tdbgRow In frm1.TDBGrid1.SelBookmarks
            frm1.rss.bookmark = tdbgRow
                ...
                rsdetail!B_name = frm1.rss!B_name
                rsdetail!B_sid = frm1.rss!B_sid
...
Private Sub choosema()
    ...
    Dim B_ps As String
    Dim B_kg As String
    Dim frm1 As New frmWhiteDelivery_Edit
     
     
     Dim sql5 As String
    Dim Rs5 As New RecordSet
    sql5 = "select * from  G_Billwhite  where B_id='" & id & "'"
    ...
    End If
    frm1.Show vbModal
     If frm1.bsaved = 1 Then
   
'        B_LoadTime = frm1.B_LoadTime
'        itemidb = frm1.itemidb
'        B_name = frm1.B_name
'        B_sid = frm1.B_sid
'        B_Width = frm1.B_Width
...
'                If rsdetail!B_LoadTime = B_LoadTime Then
...
If rsdetail.RecordCount > 0 Then
        rsdetail.MoveFirst
         ...
         b = Format(frm1.rss!B_ZCDate, "YYYY-MM-DD HH:MM:SS")
            If a = b Then
                MsgBox "有重复记录", vbInformation + vbOKOnly, "提示"
                Exit Sub
            End If
  ...
  End If
        ...
        For Each tdbgRow In frm1.TDBGrid1.SelBookmarks
            frm1.rss.bookmark = tdbgRow
                   rsdetail.AddNew
                    rsdetail!B_ZCDate = frm1.rss!B_ZCDate
                    rsdetail!B_ItemIDB = frm1.rss!B_ItemIDB
                    rsdetail!B_name = frm1.rss!B_name
                    rsdetail!B_sid = frm1.rss!B_sid
                    rsdetail!B_Width = frm1.rss!B_Width
                    rsdetail!B_weight = frm1.rss!B_UnitWeight
                    rsdetail!B_Freight = 0
                    rsdetail!B_PIShu = frm1.rss!sum1
                    rsdetail!B_kg = frm1.rss!Sum2
                    
                    rsdetail!B_ItemID = frm1.rss!B_ItemID
                    m_JudegDraft = True
                    
                    rsdetail.Update
        Next
    ElseIf frm1.bsaved = 2 Then
       Dim rs As New RecordSet
       Set rs = frm1.rss.Clone
       rs.MoveFirst
       Do While Not rs.EOF
            rsdetail.AddNew
            rsdetail!B_ZCDate = rs!B_ZCDate
            rsdetail!B_ItemIDB = rs!B_ItemIDB
            rsdetail!B_name = rs!B_name
            rsdetail!B_sid = rs!B_sid
            rsdetail!B_Width = rs!B_Width
            rsdetail!B_weight = rs!B_UnitWeight
            rsdetail!B_Freight = 0
            rsdetail!B_PIShu = rs!sum1
            rsdetail!B_kg = rs!Sum2
...
'    rsDetail.AddNew
'    rsDetail!B_LoadTime = B_LoadTime
'    rsDetail!B_ItemIDB = itemidb
'    rsDetail!B_name = B_name
'    rsDetail!B_sid = B_sid
'    rsDetail!B_Width = B_Width
...
If TDBGrid1.Columns("B_process").colIndex = colIndex Then
    ...
    If TDBGrid1.Columns("B_name").colIndex = colIndex Then
        Dim frm3 As New frmpopupWhite
        frm3.Show vbModal
        rsdetail!B_sid = frm3.Whiteid
        rsdetail!B_name = frm3.WhiteName
        Unload frm3
    End If
...
Private Sub save()
    ...
    rsdetail.MoveFirst
    Do While Not rsdetail.EOF
        If IIf(IsNull(rsdetail!B_process), "", rsdetail!B_process) = "" Then
        ...
        If IIf(IsNull(rsdetail!B_kg), "", rsdetail!B_kg) = "" Or rsdetail!B_kg = 0 Then
            ...
            Exit Sub
        End If
        If IIf(IsNull(rsdetail!B_sid), "", rsdetail!B_sid) = "" Then
            MsgBox "第" & i & "行白坯不能为空", vbInformation, "提示"
            Exit Sub
        End If
'        If rsdetail!B_ItemIDB = "" Then
'            MsgBox "第" & i & "行订单号不能为空", vbInformation, "提示"
'            Exit Sub
'        End If
    ...
    sql2 = "select * from G_billwhite where B_id='" & id & "'"
    rs2.Open sql2, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    ...
    sql1 = sql1 & "'12B010','WHT08 ','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
...
Private Sub savedetail()
    ...
    Do While Not rsdetail.EOF
        ...
        sql2 = "exec usp_insertwhitedelivery '" & item & "','" & id & "','" & rsdetail!B_ItemIDB & "'"
        sql2 = sql2 & ",'" & rsdetail!B_sid & "','" & rsdetail!B_Width & "','" & rsdetail!B_weight & "','" & rsdetail!B_PIShu & "'"
        sql2 = sql2 & ",'" & rsdetail!B_kg & "','" & rsdetail!B_processid & "','" & rsdetail!B_memo & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_qty & "','" & rsdetail!B_DeliveryNote & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_YarnSpecification & "' ,'" & rsdetail!B_Clientid & "'"
        ...
        Dim sql3 As String
        sql3 = "UPDATE G_whitecurly SET B_FPDID ='" & item & "',B_JudegFaHuo=2  WHERE B_BCitemid='" & rsdetail!B_ItemID & "' AND B_JudegFaHuo=1 AND B_ZCDate='" & rsdetail!B_ZCDate & "'"
       Debug.Print sql3
        Gm.cnnTool.cnn.Execute sql3
...
Private Sub setOk()
    ...
    rsdetail.MoveFirst
    Do While Not rsdetail.EOF
        If Len(rsdetail!B_ZCDate) > 0 Then
            sql = "update G_whitecurly set B_dl=1 where B_ZCDate='" & rsdetail!B_ZCDate & "'"
...
'        End If
    ...
    End If
        ...
        Dim sql3 As String
         sql3 = "exec usp_whitedeliveryprint '" & id & "','" & Gm.SysID.SystemUserName & "'"
         Debug.Print sql
...
Private Sub MoveFirst()
    ...
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B010' and  B_BillType='WHT08'"
...
Private Sub MovePrevious()
    ...
    sql = sql & " from G_BillWhite a left outer join G_ContactCompany b on a.B_Shipment=b.B_ClientID"
    sql = sql & " left outer join G_Employee c on a.B_Hand=c.B_SID where B_ObjectID='12B010' and  B_BillType='WHT08' and B_ID<'" & id & "'"
...
Public Sub openbill()
   Dim sql As String
   Dim rs As New RecordSet
   sql = "select B_ItemID,B_ItemIDB,B_GoodsID,b.B_Name,B_Width,B_UnitWeight,B_pishu,B_kg,B_Supplier,c.B_ClientName, B_MemoDetail,B_Waitfreight,B_qty,B_DeliveryNote,B_freight,B_Prepaidfreight,B_YarnSpecification,e.B_ClientName as B_clientkh,a.B_clientid"
   sql = sql & " from G_BillDetailWhite a left outer join G_White b on a.B_GoodsID=b.B_SID "
   sql = sql & " left outer join G_ContactCompany c  on a.B_Supplier=c.B_ClientID left outer join G_ContactCompany e on a.B_Clientid=e.B_ClientID where B_ID='" & id & "'"
   ...
   Do While Not rs.EOF
       ...
       rsdetail!B_processid = rs!B_supplier
        ...
        rsdetail!B_YarnSpecification = IIf(IsNull(rs!B_YarnSpecification), "", rs!B_YarnSpecification)
...
Private Sub saveupdate1()
    ...
    Dim a As String
     a = Format(DTPicker1.Value, "YYYY-MM-DD")
    sql1 = "exec usp_whitedeliveryupdate  '" & id & "','" & FlatEdit3.Text & "','" & a & "','WHT',"
    sql1 = sql1 & "'12B010','WHT08 ','" & Gm.SysID.SystemUser & "','" & FlatEdit2.Text & "'"
    sql1 = sql1 & ",'" & FlatEdit6.Text & "','" & FlatEdit4.Text & "','" & ComboBox1.Text & "','" & Originalsuppliers & "','" & fh & "','" & ComboBox2.Text & "','" & FlatEdit8.Text & "'"
    Gm.cnnTool.cnn.Execute sql1
    savedetailupdate
...
Private Sub savedetailupdate()
    ...
    Do While Not rsdetail.EOF
        ...
        If rs3.RecordCount > 0 Then
            sql2 = "exec usp_whitedeliveryupdate_Edit '" & rsdetail!B_ItemID & "','" & id & "','" & rsdetail!B_ItemIDB & "'"
            sql2 = sql2 & ",'" & rsdetail!B_sid & "','" & rsdetail!B_Width & "','" & rsdetail!B_weight & "','" & rsdetail!B_PIShu & "'"
            sql2 = sql2 & ",'" & rsdetail!B_kg & "','" & rsdetail!B_processid & "','" & rsdetail!B_memo & "','" & rsdetail!B_Waitfreight & "','" & rsdetail!B_Freight & "','" & rsdetail!B_Prepaidfreight & "','" & rsdetail!B_YarnSpecification & "' ,'" & rsdetail!B_Clientid & "'"
'            sql2 = sql2 & ",'" & rsDetail!B_qty & "','" & rsDetail!B_DeliveryNote & "'"
            Gm.cnnTool.cnn.Execute sql2
        Else
            saveadddetail
        End If
...
sql2 = "SELECT * FROM G_whitecurly WHERE B_ZCDate='" & rsdetail!B_ZCDate & "'"
rs2.Open sql2, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
...
sql1 = "UPDATE G_whitecurly SET B_DataPrint='" & m_SJ & "' WHERE B_ItemID='" & rs2!B_ItemID & "'"
Gm.cnnTool.cnn.Execute sql1
...
Loop
    ...
    Dim sql As String
    Dim rs As New RecordSet
    sql = "exec usp_PrintWhiteCurly_ERPXMD '" & m_SJ & "'"
    rs.Open sql, Gm.cnnTool.cnn, adOpenKeyset, adLockPessimistic
    Debug.Print sql
