...
Begin VB.Form frmWhiteDeliveryReport 
   Caption         =   "白坯发货序时表"
   ClientHeight    =   7515
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 ActiveBar21 
      ...
      _ExtentY        =   13044
      _DataPath       =   ""
      Bands           =   "frmWhiteDeliveryReport.frx":0000
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         FloodStyle      =   1
         _GridInfo       =   $"frmWhiteDeliveryReport.frx":291A
         AccessibleName  =   ""
   ...
   End
End
Attribute VB_Name = "frmWhiteDeliveryReport"
...
Private Sub Grid()
    ...
    sql = "exec usp_WhiteDeliveryReport '" & a & "','" & b & "','" & FlatEdit1.Text & "','" & Originalsuppliers & "','" & Gm.SysID.SystemUserName & "','" & FlatEdit2.Text & "','" & m_DateFormat & "'"
    ...
    For i = 0 To TDBGrid1.Columns.Count - 1
        If TDBGrid1.Columns(i).width > 0 Then
            If TDBGrid1.Columns(i).Visible = True Then
                ...
                strSQL = "update G_BLSField set B_GridWidth='" & dWidth & "' where B_ObjectID='11S039' and B_FieldName='" & szFieldName & "'"
...
Private Sub TDBGrid1_DblClick()
    ...
    If Trim(rs!B_BillType) = "WHT08" Then '染厂出库
        openbill rss!B_id
    End If
    If Trim(rs!B_BillType) = "WHT06" Then '染厂退白坯
        openbill1 rss!B_id
    End If
...
'        Exit Sub
'    End If
    Dim frm1 As New frmWhiteDelivery
    frm1.id = rs!B_id
...
'        Exit Sub
'    End If
    Dim frm1 As New frmWhiteReturn
    frm1.id = rs!B_id
    frm1.FlatEdit3.Text = IIf(IsNull(rs!B_Codeid), "", rs!B_Codeid)
    frm1.DTPicker1.Value = rs!B_Date
    frm1.FlatEdit2.Text = IIf(IsNull(rs!B_Freight), "", rs!B_Freight)
